...
func TestAccNetworkACL_basic(t *testing.T) {
	...
	resourceKey := "hcso_network_acl.fw_1"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkACLExists("hcso_network_acl.fw_1", &fwGroup),
...
func testAccCheckNetworkACLDestroy(s *terraform.State) error {
	...
	fwClient, err := config.FwV2Client(HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloud fw client: %s", err)
	}
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_network_acl" {
			continue
		}
...
resource "hcso_vpc_subnet" "subnet_2" {
	name = "%s_subnet_2"
...
resource "hcso_network_acl" "fw_1" {
  name        = "%s"
  description = "created by terraform test acc"

  inbound_rules = [hcso_network_acl_rule.rule_1.id]
  subnets = [hcso_vpc_subnet.subnet_1.id]
}
