...
func TestAccApiGatewayAPI_basic(t *testing.T) {
	var resName = "hcso_api_gateway_api.acc_apigw_api"
...
func testAccCheckApiGatewayApiDestroy(s *terraform.State) error {
	...
	apigwClient, err := config.ApiGatewayV1Client(HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloud api gateway client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_api_gateway_api" {
			continue
		}
...
func testAccApigwAPI_basic(rName string) string {
	return fmt.Sprintf(`
resource "hcso_api_gateway_group" "acc_apigw_group" {
  name        = "%s"
  description = "created by acc test"
}

resource "hcso_api_gateway_api" "acc_apigw_api" {
  group_id    = hcso_api_gateway_group.acc_apigw_group.id
  name        = "%s"
  visibility  = 2
  description = "created by acc test"
  tags        = ["tag1","tag2"]
