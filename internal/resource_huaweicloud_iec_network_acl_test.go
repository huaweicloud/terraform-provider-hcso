...
func TestAccIecNetworkACLResource_basic(t *testing.T) {
	...
	resourceKey := "hcso_iec_network_acl.acl_demo"
...
func testAccCheckIecNetworkACLDestroy(s *terraform.State) error {
	...
	iecV1Client, err := config.IECV1Client(HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating Huaweicloud IEC client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_iec_network_acl" {
			continue
		}
...
resource "hcso_iec_network_acl" "acl_demo" {
  name = "%s"
  description = "Created by terraform test acc"
  networks {
    vpc_id = hcso_iec_vpc.vpc_test.id
    subnet_id = hcso_iec_vpc_subnet.subnet_test.id
  }
}
