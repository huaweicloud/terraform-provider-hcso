...
func TestAccAOMAlarmPolicy_basic(t *testing.T) {
	...
	resourceName := "hcso_aom_alarm_policy.test"
...
func testAOMAlarmPolicy_basic(rName string) string {
	return fmt.Sprintf(`
  resource "hcso_aom_alarm_policy" "test" {
	...
	metric_name      = "hcso_sys_elb_m1_cps"
	metric_query_mode  =  "PROM"
	operator	           = ">="
	promql                = "hcso_sys_elb_m1_cps{lbaas_instance_id=\"0c9535c3-35b0-4216-8af0-19a748c423c8\"}"
	thresholds           = {
	     "Critical" = "0"
	}
	trigger_interval   =  "1m"
