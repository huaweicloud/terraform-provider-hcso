...
func TestAccAlarmActionRule_basic(t *testing.T) {
	...
	rName := "hcso_aom_alarm_action_rule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "smn_topics.0.topic_urn",
						"hcso_smn_topic.topic_1", "topic_urn"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "smn_topics.1.topic_urn",
						"hcso_smn_topic.topic_2", "topic_urn"),
				),
			},
			{
				ResourceName:      rName,
...
resource "hcso_aom_alarm_action_rule" "test" {
  ...
  smn_topics {
    topic_urn = hcso_smn_topic.topic_1.topic_urn
  }
}
`, name)
}
...
resource "hcso_smn_topic" "topic_2" {
  name = "%[1]s_2"
}
...
resource "hcso_aom_alarm_action_rule" "test" {
  ...
  smn_topics {
    topic_urn = hcso_smn_topic.topic_2.topic_urn
  }
}
`, name)
}
