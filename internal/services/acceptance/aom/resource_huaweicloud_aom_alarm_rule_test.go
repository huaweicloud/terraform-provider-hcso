...
func getAlarmRuleResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	c, err := conf.HcAomV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccAOMAlarmRule_basic(t *testing.T) {
	...
	resourceName := "hcso_aom_alarm_rule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "dimensions.0.value", "hcso_compute_instance.test", "id"),
...
resource "hcso_aom_alarm_rule" "test" {
  name                 = "%s"
  alarm_level          = 2
  ...
  dimensions {
    name  = "hostID"
    value = hcso_compute_instance.test.id
  }
