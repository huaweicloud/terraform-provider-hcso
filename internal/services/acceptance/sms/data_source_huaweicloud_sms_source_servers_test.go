...
func TestAccSourceServers_basic(t *testing.T) {
	basicDataSource := "data.hcso_sms_source_servers.all"
	byNameDataSource := "data.hcso_sms_source_servers.byName"
	nonExistentDataSource := "data.hcso_sms_source_servers.non-existent"
	basicDC := acceptance.InitDataSourceCheck(basicDataSource)
	name := acceptance.HCSO_SMS_SOURCE_SERVER
...
func testAccSourceServers_basic(name string) string {
	return fmt.Sprintf(`
data "hcso_sms_source_servers" "all" {
}

data "hcso_sms_source_servers" "byName" {
  name = "%s"
}

data "hcso_sms_source_servers" "non-existent" {
  name = "non-existent"
}
`, name)
}
