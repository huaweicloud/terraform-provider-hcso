...
func TestAccServerTemplate_basic(t *testing.T) {
	...
	resourceName := "hcso_sms_server_template.test"
	azDataName := "data.hcso_availability_zones.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "subnet_ids.0", "hcso_vpc_subnet.test", "id"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "vpc_id", "hcso_vpc.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "subnet_ids.0", "hcso_vpc_subnet.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "security_group_ids.0", "hcso_networking_secgroup.test", "id"),
				),
			},
			{
				ResourceName:      resourceName,
...
resource "hcso_sms_server_template" "test" {
  ...
  subnet_ids        = [ hcso_vpc_subnet.test.id ]
}
