...
func TestAccRAMShare_basic(t *testing.T) {
	...
	rName := "hcso_ram_resource_share.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "principals.0", acceptance.HCSO_RAM_SHARE_ACCOUNT_ID),
					resource.TestCheckResourceAttr(rName, "resource_urns.0", acceptance.HCSO_RAM_SHARE_RESOURCE_URN),
			...
			{
				// update principal
				Config: testRAMShare_basic_update(name, acceptance.HCSO_RAM_SHARE_UPDATE_ACCOUNT_ID,
					acceptance.HCSO_RAM_SHARE_RESOURCE_URN),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					...
					resource.TestCheckResourceAttr(rName, "principals.0",
						acceptance.HCSO_RAM_SHARE_UPDATE_ACCOUNT_ID),
					resource.TestCheckResourceAttr(rName, "resource_urns.0", acceptance.HCSO_RAM_SHARE_RESOURCE_URN),
			...
			},
			{
				// update resource urn
				Config: testRAMShare_basic_update(name, acceptance.HCSO_RAM_SHARE_UPDATE_ACCOUNT_ID,
					acceptance.HCSO_RAM_SHARE_UPDATE_RESOURCE_URN),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(rName, "principals.0",
						acceptance.HCSO_RAM_SHARE_UPDATE_ACCOUNT_ID),
...
`, name, acceptance.HCSO_RAM_SHARE_ACCOUNT_ID, acceptance.HCSO_RAM_SHARE_RESOURCE_URN)
}
