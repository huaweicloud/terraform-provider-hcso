...
func TestAccDatasourceAccounts_basic(t *testing.T) {
	rName := "data.hcso_organizations_accounts.all"
...
func TestAccDatasourceAccounts_name(t *testing.T) {
	rName := "data.hcso_organizations_accounts.name_filter"
	dc := acceptance.InitDataSourceCheck(rName)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDatasourceAccounts_name(acceptance.HCSO_ORGANIZATIONS_ACCOUNT_NAME),
				Check: resource.ComposeTestCheckFunc(
					dc.CheckResourceExists(),
					resource.TestCheckResourceAttrSet(rName, "accounts.#"),
...
data "hcso_organizations_accounts" "all" {}

data "hcso_organizations_accounts" "parent_id_filter" {
  parent_id = data.hcso_organizations_organization.test.root_id
}

output "parent_id_filter_is_useful" {
  value = length(data.hcso_organizations_accounts.parent_id_filter.accounts) > 0
}
`
}
...
func testAccDatasourceAccounts_name(name string) string {
	return fmt.Sprintf(`
data "hcso_organizations_accounts" "name_filter" {
  name = "%[1]s"
}

locals {
  name_filter_result = [for v in data.hcso_organizations_accounts.name_filter.accounts[*].name : v == "%[1]s"]
}
