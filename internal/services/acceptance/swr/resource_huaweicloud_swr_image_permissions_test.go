...
func TestAccSwrImagePermissions_basic(t *testing.T) {
	...
	rName := "hcso_swr_image_permissions.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "users.0.user_name",
						"hcso_identity_user.user_1", "name"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "users.2.user_id",
						"hcso_identity_user.user_4", "id"),
					resource.TestCheckResourceAttrPair(rName, "users.2.user_name",
						"hcso_identity_user.user_4", "name"),
					resource.TestCheckResourceAttr(rName, "users.2.permission", "Manage"),
					resource.TestCheckResourceAttrPair(rName, "users.3.user_id",
						"hcso_identity_user.user_5", "id"),
					resource.TestCheckResourceAttrPair(rName, "users.3.user_name",
						"hcso_identity_user.user_5", "name"),
...
resource "hcso_identity_user" "user_3" {
 name     = "%[2]s_3"
 enabled  = true
 password = "password12345!"
}
...
resource "hcso_swr_image_permissions" "test" {
 ...
 users {
   ...
   user_name  = hcso_identity_user.user_1.name
   permission = "Read"
 }

 users {
   user_id    = hcso_identity_user.user_2.id
   permission = "Write"
 }

 users {
   user_id    = hcso_identity_user.user_3.id
   permission = "Manage"
 }
}
...
resource "hcso_identity_user" "user_4" {
 name     = "%[2]s_4"
 enabled  = true
 password = "password12345!"
}

resource "hcso_identity_user" "user_5" {
 name     = "%[2]s_5"
 enabled  = true
 password = "password12345!"
}
...
resource "hcso_swr_image_permissions" "test" {
 ...
 users {
   user_id    = hcso_identity_user.user_4.id
   permission = "Manage"
 }

 users {
   user_id    = hcso_identity_user.user_5.id
   permission = "Write"
 }
} 
`, testAccSWRRepository_basic(name), name)
