...
func getVirtualInterfaceFunc(config *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := config.DcV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccVirtualInterface_basic(t *testing.T) {
	var (
		...
		rName      = "hcso_dc_virtual_interface.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "direct_connect_id", acceptance.HCSO_DC_DIRECT_CONNECT_ID),
					resource.TestCheckResourceAttrPair(rName, "vgw_id", "hcso_dc_virtual_gateway.test", "id"),
...
resource "hcso_dc_virtual_interface" "test" {
  ...
  vgw_id            = hcso_dc_virtual_gateway.test.id
  name              = "%[3]s"
  description       = "Created by acc test"
  type              = "private"
  route_mode        = "static"
...
`, testAccVirtualInterface_base(name), acceptance.HCSO_DC_DIRECT_CONNECT_ID, name, vlan)
}
