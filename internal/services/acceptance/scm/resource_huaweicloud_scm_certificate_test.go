...
func getSCMResourceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.ScmV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccScmCertification_basic(t *testing.T) {
	...
	resourceName := "hcso_scm_certificate.certificate_1"
...
func TestAccScmCertification_push(t *testing.T) {
	var certInfo certificates.CertificateEscrowInfo
	resourceName := "hcso_scm_certificate.certificate_2"

	rName := acceptance.RandomAccResourceNameWithDash()
	service := acceptance.HCSO_CERTIFICATE_SERVICE
	defaultProject := acceptance.HCSO_CERTIFICATE_PROJECT
	newProject := acceptance.HCSO_CERTIFICATE_PROJECT_UPDATED
...
func TestAccScmCertification_batchPush(t *testing.T) {
	var certInfo certificates.CertificateEscrowInfo
	resourceName := "hcso_scm_certificate.certificate_3"
...
resource "hcso_scm_certificate" "certificate_1" {
  ...
  private_key       = file("%s")
}`, name, acceptance.HCSO_CERTIFICATE_KEY_PATH, acceptance.HCSO_CERTIFICATE_CHAIN_PATH,
		acceptance.HCSO_CERTIFICATE_PRIVATE_KEY_PATH)
}
...
resource "hcso_scm_certificate" "certificate_2" {
  ...
  target {
    ...
    service = "%s"
		...
		acceptance.HCSO_CERTIFICATE_PRIVATE_KEY_PATH, project, service)
}
...
resource "hcso_scm_certificate" "certificate_3" {
  ...
  target {
    ...
    service = "%s"
		...
		acceptance.HCSO_CERTIFICATE_PRIVATE_KEY_PATH, acceptance.HCSO_CERTIFICATE_PROJECT,
		acceptance.HCSO_CERTIFICATE_PROJECT_UPDATED, acceptance.HCSO_CERTIFICATE_SERVICE)
}
