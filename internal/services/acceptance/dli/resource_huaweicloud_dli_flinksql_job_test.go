...
func TestAccResourceDliFlinkJob_basic(t *testing.T) {
	...
	resourceName := "hcso_dli_flinksql_job.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccFlinkJobResource_basic(name, acceptance.HCSO_REGION_NAME),
...
resource "hcso_dis_stream" "stream_input" {
  stream_name     = "%s_input"
  partition_count = 1
  data_type       = "CSV"
  csv_delimiter   = ","
...
resource "hcso_dis_stream" "stream_output" {
...
resource "hcso_dli_flinksql_job" "test" {
  ...
  sql  = var.sql
  depends_on = [
    hcso_dis_stream.stream_input,
    hcso_dis_stream.stream_output,
  ]
}
