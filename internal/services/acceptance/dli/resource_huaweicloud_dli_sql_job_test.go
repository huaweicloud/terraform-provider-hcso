...
func TestAccResourceDliSqlJob_basic(t *testing.T) {
	...
	resourceName := "hcso_dli_sql_job.test"
...
resource "hcso_dli_sql_job" "test" {
  sql           = "DESC ${hcso_dli_table.test.name}"
...
resource "hcso_dli_sql_job" "test" {
  sql           = "SELECT * FROM ${hcso_dli_table.test.name}"
  database_name = hcso_dli_database.test.name
...
func testAccCheckDliSqlJobDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err == nil && res != nil && (res.Status != sqljob.JobStatusCancelled &&
			...
			return fmt.Errorf("hcso_dli_sql_job still exists:%s,%+v,%+v", rs.Primary.ID, err, res)
		}
	}
