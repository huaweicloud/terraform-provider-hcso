...
func TestAccResourceDliFlinkJarJob_basic(t *testing.T) {
	...
	resourceName := "hcso_dli_flinkjar_job.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccFlinkJarJobResource_basic(name, bucketName, acceptance.HCSO_REGION_NAME,
					acceptance.HCSO_ACCESS_KEY, acceptance.HCSO_SECRET_KEY, acceptance.HCSO_DLI_FLINK_JAR_OBS_PATH),
...
resource "hcso_dli_flinkjar_job" "test" {
  name            = "%s"
  queue_name      = hcso_dli_queue.test.name
  ...
  obs_bucket      = hcso_obs_bucket.test.bucket
  log_enabled     = true
}
...
func TestAccResourceDliFlinkJarJob_all(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccFlinkJarJobResource_all(name, bucketName, acceptance.HCSO_REGION_NAME, acceptance.HCSO_ACCESS_KEY,
					acceptance.HCSO_SECRET_KEY, acceptance.HCSO_DLI_FLINK_JAR_OBS_PATH),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(resourceName, "name", name),
					resource.TestCheckResourceAttr(resourceName, "status", "job_running"),
					resource.TestCheckResourceAttr(resourceName, "queue_name", name),
					resource.TestCheckResourceAttrPair(resourceName, "obs_bucket", "hcso_obs_bucket.test", "bucket"),
					...
					resource.TestCheckResourceAttr(resourceName, "main_class", "com.internal.dli.obs.Main"),
...
resource "hcso_dli_flinkjar_job" "test" {
  ...
  main_class             = "com.internal.dli.obs.Main"
  ...
  tm_cu_num              = 2
  tm_slot_num            = 1
  smn_topic              = hcso_smn_topic.test.name
  restart_when_exception = "true"
  resume_checkpoint      = true
  resume_max_num         = 2
  checkpoint_path        = "${hcso_obs_bucket.test.bucket}/checkpoint/"
}
