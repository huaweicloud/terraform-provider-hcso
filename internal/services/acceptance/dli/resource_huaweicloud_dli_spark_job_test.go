...
func getSparkJobResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	c, err := conf.DliV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccDliSparkJobV2_basic(t *testing.T) {
	...
	resourceName := "hcso_dli_spark_job.test"
...
func testAccCheckDliSparkJobDestroy(s *terraform.State) error {
	...
	client, err := config.DliV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmt.Errorf("error creating Dli v2 client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_dli_spark_job" {
			continue
		}
...
resource "hcso_dli_spark_job" "test" {
  queue_name = hcso_dli_queue.test.name
  name       = "%s"
  app_name   = "${hcso_dli_package.test.group_name}/${hcso_dli_package.test.object_name}"
  
 depends_on = [
    hcso_obs_bucket.test,
    hcso_obs_bucket_object.test,
  ]
}
