...
func TestAccDliPackage_basic(t *testing.T) {
	...
	resourceName := "hcso_dli_package.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "object_path", fmt.Sprintf(
						...
						rName, acceptance.HCSO_REGION_NAME)),
...
resource "hcso_dli_package" "test" {
  group_name  = "%s"
  type        = "pyFile"
  object_path = "https://${hcso_obs_bucket.test.bucket_domain_name}/dli/packages/simple_pyspark_test_DLF_refresh.py"
}
`, rName, acceptance.HCSO_ACCESS_KEY, acceptance.HCSO_SECRET_KEY, rName)
}
