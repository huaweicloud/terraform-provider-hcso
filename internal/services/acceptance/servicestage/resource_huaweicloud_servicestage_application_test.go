...
func TestAccApplication_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_servicestage_application.test"
	)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "environment.0.id",
						"hcso_servicestage_environment.test", "id"),
...
resource "hcso_servicestage_application" "test" {
  ...
  environment {
    id = hcso_servicestage_environment.test.id
...
`, testAccApplication_base(rName), rName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
