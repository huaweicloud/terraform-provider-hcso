...
func TestAccComponentInstance_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_servicestage_component_instance.test"
	)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "application_id", "hcso_servicestage_application.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "component_id", "hcso_servicestage_component.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "environment_id", "hcso_servicestage_environment.test", "id"),
					...
					resource.TestCheckResourceAttrPair(resourceName, "artifact.0.name", "hcso_servicestage_component.test", "name"),
					...
					resource.TestCheckResourceAttr(resourceName, "artifact.0.url", acceptance.HCSO_BUILD_IMAGE_URL),
...
func testAccInstanceImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "hcso_servicestage_component_instance" {
...
resource "hcso_servicestage_component" "test" {
  ...
  application_id = hcso_servicestage_application.test.id
...
resource "hcso_servicestage_component_instance" "test" {
  application_id = hcso_servicestage_application.test.id
  component_id   = hcso_servicestage_component.test.id
  environment_id = hcso_servicestage_environment.test.id
  ...
  artifact {
    name      = hcso_servicestage_component.test.name
    type      = "image"
    storage   = "swr"
    url       = "%[3]s"
    auth_type = "iam"
...
`, testAccComponentInstance_base(rName), rName, acceptance.HCSO_BUILD_IMAGE_URL)
}
