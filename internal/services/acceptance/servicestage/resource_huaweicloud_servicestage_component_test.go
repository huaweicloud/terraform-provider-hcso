...
func TestAccComponent_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_servicestage_component.test"
	)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "source.0.authorization",
						"hcso_servicestage_repo_token_authorization.test", "name"),
					resource.TestCheckResourceAttr(resourceName, "source.0.url", acceptance.HCSO_GITHUB_REPO_URL),
					resource.TestCheckResourceAttr(resourceName, "builder.0.organization", acceptance.HCSO_DOMAIN_NAME),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "source.0.url", acceptance.HCSO_OBS_STORAGE_URL),
					resource.TestCheckResourceAttr(resourceName, "builder.0.organization", acceptance.HCSO_DOMAIN_NAME),
					resource.TestCheckResourceAttrPair(resourceName, "builder.0.cluster_id",
						"hcso_cce_cluster.test", "id"),
...
func testAccComponentImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "hcso_servicestage_component" {
...
`, rName, acceptance.HCSO_GITHUB_REPO_HOST, acceptance.HCSO_GITHUB_PERSONAL_TOKEN)
}
...
resource "hcso_servicestage_component" "test" {
  ...
  source {
    type          = "GitHub"
    authorization = hcso_servicestage_repo_token_authorization.test.name
    url           = "%[3]s"
    repo_ref      = "master"
  }
...
`, testAccComponent_buildConfig(rName), rName, acceptance.HCSO_GITHUB_REPO_URL, acceptance.HCSO_DOMAIN_NAME)
}
...
`, testAccComponent_buildConfig(rName), rName, acceptance.HCSO_OBS_STORAGE_URL, acceptance.HCSO_DOMAIN_NAME)
}
