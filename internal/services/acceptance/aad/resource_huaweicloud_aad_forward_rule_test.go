...
func getForwardRuleFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := conf.AadV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccForwardRule_basic(t *testing.T) {
	var rule rules.Rule
	resourceName := "hcso_aad_forward_rule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "instance_id", acceptance.HCSO_AAD_INSTANCE_ID),
					resource.TestCheckResourceAttr(resourceName, "ip", acceptance.HCSO_AAD_IP_ADDRESS),
...
`, acceptance.HCSO_AAD_INSTANCE_ID, acceptance.HCSO_AAD_IP_ADDRESS, port)
}
