...
func TestAccIdentityCenterAccountAssignment_basic(t *testing.T) {
	...
	rName := "hcso_identitycenter_account_assignment.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "instance_id",
						"data.hcso_identitycenter_instance.test", "id"),
					...
					resource.TestCheckResourceAttrPair(rName, "principal_id",
						"hcso_identitycenter_user.test", "id"),
					resource.TestCheckResourceAttr(rName, "principal_type", "USER"),
					resource.TestCheckResourceAttr(rName, "target_id", acceptance.HCSO_IDENTITY_CENTER_ACCOUNT_ID),
				),
			},
			{
				ResourceName:      rName,
...
resource "hcso_identitycenter_permission_set" "test" {
  instance_id      = data.hcso_identitycenter_instance.test.id
  name             = "%[2]s"
...
resource "hcso_identitycenter_account_assignment" "test" {
  ...
  principal_id      = hcso_identitycenter_user.test.id
  principal_type    = "USER"
  target_id         = "%[3]s"
  target_type       = "ACCOUNT"
}
`, testIdentityCenterUser_basic(name), name, acceptance.HCSO_IDENTITY_CENTER_ACCOUNT_ID)
}
