...
func getDcsResourceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.DcsV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccDcsInstances_basic(t *testing.T) {
	...
	resourceName := "hcso_dcs_instance.instance_1"
...
func TestAccDcsInstances_prePaid(t *testing.T) {
	...
	resourceName := "hcso_dcs_instance.test"
...
resource "hcso_dcs_instance" "instance_1" {
  ...
  flavor             = data.hcso_dcs_flavors.test.flavors[0].name
...
}`, instanceName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
...
resource "hcso_dcs_instance" "test" {
  vpc_id             = data.hcso_vpc.test.id
  subnet_id          = data.hcso_vpc_subnet.test.id
  availability_zones = try(slice(data.hcso_availability_zones.test.names, 0, 1), [])
  name               = "%s"
  engine             = "Redis"
  engine_version     = "5.0"
  flavor             = data.hcso_dcs_flavors.test.flavors[0].name
