...
func TestAccRecording_basic(t *testing.T) {
	...
	pushDomainName := fmt.Sprintf("%s.internal.com", name)
	rName := "hcso_live_recording.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "obs.0.region", acceptance.HCSO_REGION_NAME),
...
resource "hcso_live_recording" "test" {
  ...
  obs {
    region = hcso_obs_bucket.bucket.region
