...
func getDomainResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := conf.HcLiveV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccDomain_basic(t *testing.T) {
	...
	pushDomainName := fmt.Sprintf("%s.internal.com", acceptance.RandomAccResourceNameWithDash())
	pullDomainName := fmt.Sprintf("%s.internal.com", acceptance.RandomAccResourceNameWithDash())
	pushResourceName := "hcso_live_domain.ingestDomain"
	pullResourceName := "hcso_live_domain.streamingDomain"
