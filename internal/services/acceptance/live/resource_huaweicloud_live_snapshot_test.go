...
func TestAccLiveSnapshot_basic(t *testing.T) {
	...
	domainName := fmt.Sprintf("%s.internal.com", name)
	rName := "hcso_live_snapshot.test"
...
func testLiveSnapshot_basic(domainName, name string) string {
	return fmt.Sprintf(`
resource "hcso_live_domain" "test" {
  name = "%[1]s"
  type = "push"
}
...
resource "hcso_live_snapshot" "test" {
  depends_on = [hcso_live_bucket_authorization.test]

  domain_name       = hcso_live_domain.test.name
  app_name          = "live"
  frequency         = 10
  storage_mode      = 0
  storage_bucket    = hcso_obs_bucket.test.bucket
  storage_path      = "input/"
...
resource "hcso_obs_bucket" "test_update" {
  bucket        = "%[2]s-update"
  acl           = "private"
  force_destroy = true
}

resource "hcso_live_bucket_authorization" "test_update" {
  depends_on = [hcso_obs_bucket.test_update]

  bucket = "%[2]s-update"
}

resource "hcso_live_snapshot" "test" {
  depends_on = [hcso_live_bucket_authorization.test_update]

  domain_name        = hcso_live_domain.test.name
  app_name           = "live"
  frequency          = 20
  storage_mode       = 1
  storage_bucket     = hcso_obs_bucket.test_update.bucket
  storage_path       = "output/"
