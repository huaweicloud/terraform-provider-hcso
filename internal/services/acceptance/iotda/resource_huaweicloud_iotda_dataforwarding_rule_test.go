...
func TestAccDataForwardingRule_basic(t *testing.T) {
	...
	rName := "hcso_iotda_dataforwarding_rule.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testDataForwardingRule_dis(updateName, acceptance.HCSO_REGION_NAME),
				Check: resource.ComposeTestCheckFunc(
...
resource "hcso_iotda_dataforwarding_rule" "test" {
  ...
  targets {
    ...
    dis_forwarding {
      region    = "%s"
      stream_id = hcso_dis_stream.test.id
    }
  }
}
`, name, name, region)
