...
func TestAccDevice_basic(t *testing.T) {
	...
	rName := "hcso_iotda_device.test"
	childDeviceName := "hcso_iotda_device.test2"
...
resource "hcso_iotda_device" "test" {
  node_id     = "%[3]s"
  name        = "%[2]s"
  space_id    = hcso_iotda_space.test.id
  product_id  = hcso_iotda_product.test.id
  secret      = "1234567890"
  description = "demo"
...
resource "hcso_iotda_device" "test2" {
  node_id    = "%[3]s_2"
  name       = "%[2]s_2"
  space_id   = hcso_iotda_space.test.id
  product_id = hcso_iotda_product.test.id
  gateway_id = hcso_iotda_device.test.id
}
