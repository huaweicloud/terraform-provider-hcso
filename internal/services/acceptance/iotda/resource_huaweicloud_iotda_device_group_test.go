...
func TestAccDeviceGroup_basic(t *testing.T) {
	...
	rName := "hcso_iotda_device_group.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "device_ids.0", "hcso_iotda_device.test", "id"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "space_id", "hcso_iotda_space.test", "id"),
					resource.TestCheckResourceAttr(rName, "device_ids.#", "1"),
					resource.TestCheckResourceAttrPair(rName, "device_ids.0", "hcso_iotda_device.test2", "id"),
				),
			},
			{
				ResourceName:            rName,
...
resource "hcso_iotda_device_group" "test" {
  ...
  description = "%s"
  device_ids  = [hcso_iotda_device.test.id]
}
...
resource "hcso_iotda_device_group" "test" {
  ...
  space_id    = hcso_iotda_space.test.id
  description = "%s"
  device_ids  = [hcso_iotda_device.test2.id]
}
`, baseConfig, name, name)
}
