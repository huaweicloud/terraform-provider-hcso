...
func getDeviceLinkageRuleResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := conf.HcIoTdaV5Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccDeviceLinkageRule_basic(t *testing.T) {
	...
	rName := "hcso_iotda_device_linkage_rule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "space_id", "hcso_iotda_space.test", "id"),
...
resource "hcso_iotda_device_linkage_rule" "test" {
  ...
  triggers {
    ...
    device_data_condition {
      product_id            = hcso_iotda_device.test.product_id
      path                  = "service_1/p_1"
      operator              = "="
      value                 = 4
  ...
  depends_on = [
    hcso_iotda_device.test,
    hcso_iotda_product.test,
  ]
}
...
resource "hcso_iotda_device_linkage_rule" "test" {
  ...
  actions {
    type = "DEVICE_CMD"
    device_command {
      device_id    = hcso_iotda_device.test.id
