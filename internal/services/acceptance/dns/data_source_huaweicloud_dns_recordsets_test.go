...
func TestAccDatasourceDNSRecordsets_basic(t *testing.T) {
	rName := "data.hcso_dns_recordsets.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.hcso_dns_recordsets.tags_filter", "recordsets.#", "1"),
				),
			},
		},
	})
...
data "hcso_dns_recordsets" "test" {
  zone_id = hcso_dns_recordset.test.zone_id
}

data "hcso_dns_recordsets" "line_id_filter" {
  zone_id = hcso_dns_recordset.test.zone_id
  line_id = hcso_dns_recordset.test.line_id
}
data "hcso_dns_recordsets" "status_filter" {
  zone_id = hcso_dns_recordset.test.zone_id
  status  = "ACTIVE"
}
data "hcso_dns_recordsets" "type_filter" {
  zone_id = hcso_dns_recordset.test.zone_id
  type    = hcso_dns_recordset.test.type
}
data "hcso_dns_recordsets" "name_filter" {
  zone_id = hcso_dns_recordset.test.zone_id
  name    = hcso_dns_recordset.test.name
}
data "hcso_dns_recordsets" "recordset_id_filter" {
  zone_id      = hcso_dns_recordset.test.zone_id
  recordset_id = split("/", hcso_dns_recordset.test.id).1
}

locals {
  line_id_filter_result = [for v in data.hcso_dns_recordsets.line_id_filter.recordsets[*].line_id :
v == hcso_dns_recordset.test.line_id]
  status_filter_result = [for v in data.hcso_dns_recordsets.status_filter.recordsets[*].status : v == "ACTIVE"]
  type_filter_result = [for v in data.hcso_dns_recordsets.type_filter.recordsets[*].type : v == hcso_dns_recordset.test.type]
  name_filter_result = [for v in data.hcso_dns_recordsets.name_filter.recordsets[*].name : v == hcso_dns_recordset.test.name]
  recordset_id_filter_result = [for v in data.hcso_dns_recordsets.recordset_id_filter.recordsets[*].id :
v == split("/", hcso_dns_recordset.test.id).1]
}
...
data "hcso_dns_recordsets" "tags_filter" {
  zone_id = hcso_dns_recordset.test.zone_id
  tags    = "key1,value1"
}
