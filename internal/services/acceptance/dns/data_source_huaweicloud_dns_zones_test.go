...
func TestAccDatasourceDNSZones_basic(t *testing.T) {
	rName := "data.hcso_dns_zones.test"
...
data "hcso_dns_zones" "test" {
  zone_type = hcso_dns_zone.zone_1.zone_type
}

data "hcso_dns_zones" "tags_filter" {
  zone_type = hcso_dns_zone.zone_1.zone_type
  tags      = "zone_type,private"
}
data "hcso_dns_zones" "name_filter" {
  zone_type = hcso_dns_zone.zone_1.zone_type
  name      = hcso_dns_zone.zone_1.name
}
data "hcso_dns_zones" "status_filter" {
  zone_type = hcso_dns_zone.zone_1.zone_type
  status    = data.hcso_dns_zones.test.zones.0.status
}
data "hcso_dns_zones" "enterprise_project_id_filter" {
  zone_type             = hcso_dns_zone.zone_1.zone_type
  enterprise_project_id = hcso_dns_zone.zone_1.enterprise_project_id
}

locals {
  tags_filter_result = [for v in data.hcso_dns_zones.tags_filter.zones[*].tags : v.zone_type == "private"]
  name_filter_result = [for v in data.hcso_dns_zones.name_filter.zones[*].name : v == hcso_dns_zone.zone_1.name]
  status_filter_result = [for v in data.hcso_dns_zones.status_filter.zones[*].status : v == data.hcso_dns_zones.test.zones.0.status]
  enterprise_project_id_filter_result = [for v in data.hcso_dns_zones.enterprise_project_id_filter.zones[*].enterprise_project_id :
v == hcso_dns_zone.zone_1.enterprise_project_id]
}
...
locals {
  tags_filter_result = [for v in data.hcso_dns_zones.tags_filter.zones[*].tags : v.zone_type == "public"]
  name_filter_result = [for v in data.hcso_dns_zones.name_filter.zones[*].name : v == hcso_dns_zone.zone_1.name]
  status_filter_result = [for v in data.hcso_dns_zones.status_filter.zones[*].status : v == data.hcso_dns_zones.test.zones.0.status]
  enterprise_project_id_filter_result = [for v in data.hcso_dns_zones.enterprise_project_id_filter.zones[*].enterprise_project_id :
v == hcso_dns_zone.zone_1.enterprise_project_id]
