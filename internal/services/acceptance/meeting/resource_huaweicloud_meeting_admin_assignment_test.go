...
func TestAccAdminAssignment_basic(t *testing.T) {
	var (
		...
		resourceName  = "hcso_meeting_admin_assignment.test"
	)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "account", "hcso_meeting_user.test", "account"),
				),
			},
			{
...
func testAccAdminAssignmentImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "hcso_meeting_admin_assignment" {
...
resource "hcso_meeting_admin_assignment" "test" {
  ...
  account = hcso_meeting_user.test.account
}
