...
func TestAccDataCustomEventSources_basic(t *testing.T) {
	var (
		baseRes      = "hcso_eg_custom_event_source.test"
		byName       = "data.hcso_eg_custom_event_sources.filter_by_name"
		nameNotFound = "data.hcso_eg_custom_event_sources.name_not_found"
...
`, acceptance.HCSO_EG_CHANNEL_ID, name)
}
...
data "hcso_eg_custom_event_sources" "filter_by_name" {
  // The behavior of parameter 'name' of the resource is 'Required', means this parameter does not have 'Know After Apply' behavior.
  depends_on = [
    hcso_eg_custom_event_source.test,
  ]

  name = hcso_eg_custom_event_source.test.name
}

data "hcso_eg_custom_event_sources" "name_not_found" {
...
locals {
  filter_result = [for v in data.hcso_eg_custom_event_sources.filter_by_name.sources[*].id :
                   v == hcso_eg_custom_event_source.test.id]
}
...
output "name_not_found_validation_pass" {
  value = length(data.hcso_eg_custom_event_sources.name_not_found.sources) == 0
}
...
func TestAccDataCustomEventSources_filterById(t *testing.T) {
	var (
		baseRes    = "hcso_eg_custom_event_source.test"
		byId       = "data.hcso_eg_custom_event_sources.filter_by_id"
		idNotFound = "data.hcso_eg_custom_event_sources.id_not_found"
...
data "hcso_eg_custom_event_sources" "filter_by_id" {
  source_id = hcso_eg_custom_event_source.test.id
}

data "hcso_eg_custom_event_sources" "id_not_found" {
...
locals {
  filter_result = [for v in data.hcso_eg_custom_event_sources.filter_by_id.sources[*].id :
                   v == hcso_eg_custom_event_source.test.id]
}
...
output "id_not_found_validation_pass" {
  value = length(data.hcso_eg_custom_event_sources.id_not_found.sources) == 0
}
...
func TestAccDataCustomEventSources_filterByChannelId(t *testing.T) {
	var (
		baseRes           = "hcso_eg_custom_event_source.test"
		byChannelId       = "data.hcso_eg_custom_event_sources.filter_by_channel_id"
		channelIdNotFound = "data.hcso_eg_custom_event_sources.channel_id_not_found"
...
data "hcso_eg_custom_event_sources" "filter_by_channel_id" {
...
data "hcso_eg_custom_event_sources" "channel_id_not_found" {
  // Since a random ID is used, there is no dependency relationship with resource attachment, and the dependency needs
  // to be manually set.
  depends_on = [
    hcso_eg_custom_event_source.test,
...
locals {
  filter_result = [for v in data.hcso_eg_custom_event_sources.filter_by_channel_id.sources[*].channel_id : v == "%[2]s"]
}
...
output "channel_id_not_found_validation_pass" {
  value = length(data.hcso_eg_custom_event_sources.channel_id_not_found.sources) == 0
}
`, testAccDataCustomEventSources_base(), acceptance.HCSO_EG_CHANNEL_ID, randUUID)
}
