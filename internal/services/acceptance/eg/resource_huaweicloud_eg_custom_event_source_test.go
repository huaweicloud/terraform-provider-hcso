...
func getCustomEventSourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.EgV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccCustomEventSource_basic(t *testing.T) {
	var (
		...
		rName = "hcso_eg_custom_event_source.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "channel_id", acceptance.HCSO_EG_CHANNEL_ID),
