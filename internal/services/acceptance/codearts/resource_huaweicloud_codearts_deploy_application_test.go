...
func TestAccDeployApplication_basic(t *testing.T) {
	...
	rName := "hcso_codearts_deploy_application.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "resource_pool_id", acceptance.HCSO_CODEARTS_RESOURCE_POOL_ID),
					resource.TestCheckResourceAttr(rName, "steps.step1", "Download Package"),
					resource.TestCheckResourceAttrSet(rName, "created_at"),
					resource.TestCheckResourceAttrSet(rName, "updated_at"),
					resource.TestCheckResourceAttrSet(rName, "project_name"),
...
`, testProject_basic(name), name, acceptance.HCSO_CODEARTS_RESOURCE_POOL_ID)
}
