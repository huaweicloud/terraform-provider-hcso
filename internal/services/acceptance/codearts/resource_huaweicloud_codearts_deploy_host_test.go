...
func TestAccDeployHost_withProxyMode(t *testing.T) {
	...
	rName := "hcso_codearts_deploy_host.test"
	proxyRName := "hcso_codearts_deploy_host.test_proxy"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "group_id",
						"hcso_codearts_deploy_group.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "ip_address",
						"hcso_compute_instance.test.1", "public_ip"),
					resource.TestCheckResourceAttrPair(rName, "proxy_host_id",
						"hcso_codearts_deploy_host.test_proxy", "id"),
					...
					resource.TestCheckResourceAttrPair(proxyRName, "ip_address",
						"hcso_compute_instance.test.0", "public_ip"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttrPair(rName, "ip_address",
						"hcso_compute_instance.test.1", "access_ip_v4"),
...
resource "hcso_codearts_deploy_group" "test" {
  project_id  = hcso_codearts_project.test.id
  name        = "%[3]s"
  os_type     = "linux"
...
resource "hcso_codearts_deploy_host" "test_proxy" {
  group_id   = hcso_codearts_deploy_group.test.id
  ip_address = hcso_compute_instance.test[0].public_ip
  port       = 22
  username   = "root"
  password   = "Test@123"
  os_type    = "linux"
...
resource "hcso_codearts_deploy_host" "test" {
  group_id        = hcso_codearts_deploy_group.test.id
  ip_address      = hcso_compute_instance.test[1].public_ip
  port            = 22
  username        = "root"
  password        = "Test@123"
  os_type         = "linux"
  proxy_host_id   = hcso_codearts_deploy_host.test_proxy.id
  name            = "%[3]s"
...
resource "hcso_codearts_deploy_host" "test" {
  group_id        = hcso_codearts_deploy_group.test.id
  ip_address      = hcso_compute_instance.test[1].access_ip_v4
  port            = 22
  username        = "root"
  password        = "Test@123"
  os_type         = "linux"
