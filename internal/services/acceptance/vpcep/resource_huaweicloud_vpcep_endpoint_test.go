...
func TestAccVPCEndpoint_Basic(t *testing.T) {
	...
	resourceName := "hcso_vpcep_endpoint.test"
...
func TestAccVPCEndpoint_Public(t *testing.T) {
	var endpoint endpoints.Endpoint
	resourceName := "hcso_vpcep_endpoint.myendpoint"
	rc := acceptance.InitResourceCheck(
		resourceName,
		&endpoint,
		getVpcepEndpointResourceFunc,
...
func getVpcepEndpointResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	vpcepClient, err := conf.VPCEPClient(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
resource "hcso_compute_instance" "ecs" {
  name               = "%s"
...
resource "hcso_vpcep_service" "test" {
  name        = "%s"
  server_type = "VM"
  vpc_id      = data.hcso_vpc.myvpc.id
  port_id     = hcso_compute_instance.ecs.network[0].port
  approval    = false
