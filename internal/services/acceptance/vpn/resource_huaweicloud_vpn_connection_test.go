...
func TestAccConnection_basic(t *testing.T) {
	...
	rName := "hcso_vpn_connection.test"
...
resource "hcso_vpn_connection" "test" {
  name                = "%s"
  gateway_id          = hcso_vpn_gateway.test.id
  gateway_ip          = hcso_vpn_gateway.test.master_eip[0].id
  customer_gateway_id = hcso_vpn_customer_gateway.test.id
