		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "connect_subnet", "hcso_vpc_subnet.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "vpc_id", "hcso_vpc.test", "id"),
					...
					resource.TestCheckResourceAttrPair(rName, "local_subnets.0", "hcso_vpc_subnet.test", "cidr"),
					resource.TestCheckResourceAttrPair(rName, "master_eip.0.id", "hcso_vpc_eip.test1", "id"),
					resource.TestCheckResourceAttrPair(rName, "slave_eip.0.id", "hcso_vpc_eip.test2", "id"),
				),
			},
			{
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "status", "ACTIVE"),
					resource.TestCheckResourceAttrPair(rName, "er_id", "hcso_er_instance.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "access_vpc_id", "hcso_vpc.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "access_subnet_id", "hcso_vpc_subnet.test", "id"),
...
resource "hcso_vpn_gateway" "test" {
  name               = "%s"
  vpc_id             = hcso_vpc.test.id
  local_subnets      = [hcso_vpc_subnet.test.cidr]
  connect_subnet     = hcso_vpc_subnet.test.id
  availability_zones = [
    data.hcso_availability_zones.test.names[0],
    data.hcso_availability_zones.test.names[1]
  ]
  ...
  master_eip {
    id = hcso_vpc_eip.test1.id
  }

  slave_eip {
    id = hcso_vpc_eip.test2.id
  }
}
...
resource "hcso_vpn_gateway" "test" {
  name               = "%s-update"
  vpc_id             = hcso_vpc.test.id
  local_subnets      = [hcso_vpc_subnet.test.cidr, "192.168.2.0/24"]
  connect_subnet     = hcso_vpc_subnet.test.id
  availability_zones = [
    data.hcso_availability_zones.test.names[0],
    data.hcso_availability_zones.test.names[1]
...
resource "hcso_vpn_gateway" "test" {
  ...
  attachment_type    = "er"
  er_id              = hcso_er_instance.test.id
  availability_zones = [
    data.hcso_availability_zones.test.names[0],
    data.hcso_availability_zones.test.names[1]
  ]

  access_vpc_id    = hcso_vpc.test.id
  access_subnet_id = hcso_vpc_subnet.test.id
}
`, name)
}
