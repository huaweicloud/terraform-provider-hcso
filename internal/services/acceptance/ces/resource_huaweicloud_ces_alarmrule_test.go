...
func getAlarmRuleResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	c, err := conf.CesV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccCESAlarmRule_basic(t *testing.T) {
	...
	resourceName := "hcso_ces_alarmrule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "resource_group_id",
						"hcso_ces_resource_group.test", "id"),
				),
			},
		},
	})
...
resource "hcso_ces_alarmrule" "test" {
  ...
  resources {
    dimensions {
      name  = "instance_id"
      value = hcso_compute_instance.test[0].id
    }
  }
...
resource "hcso_ces_alarmrule" "test" {
  ...
  resources {
    dimensions {
      name  = "instance_id"
      value = hcso_compute_instance.test[1].id
    }
  }
...
`, testCESAlarmRule_instanceBase(rName), testCESAlarmRule_topicBase(rName), rName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
...
resource "hcso_ces_alarmrule" "test" {
  ...
  resource_group_id    = hcso_ces_resource_group.test.id
