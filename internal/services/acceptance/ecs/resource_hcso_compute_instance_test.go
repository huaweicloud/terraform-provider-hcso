...
func TestAccComputeInstance_withEPS(t *testing.T) {
	...
	srcEPS := acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST
	destEPS := acceptance.HCSO_ENTERPRISE_MIGRATE_PROJECT_ID_TEST
	rName := acceptance.RandomAccResourceName()
	resourceName := "hcso_compute_instance.test"
...
func testAccCheckComputeInstanceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_compute_instance" {
			continue
		}
