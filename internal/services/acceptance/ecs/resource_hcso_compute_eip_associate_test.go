...
func TestAccComputeEIPAssociate_basic(t *testing.T) {
	...
	resourceName := "hcso_compute_eip_associate.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckVpcV1EIPExists("hcso_vpc_eip.test", &eip),
					testAccCheckComputeEIPAssociateAssociated(&eip, &instance),
					resource.TestCheckResourceAttrSet(resourceName, "port_id"),
					resource.TestCheckResourceAttrPair("data.hcso_compute_instance.test", "public_ip",
...
func testAccCheckComputeEIPAssociateDestroy(s *terraform.State) error {
	...
	computeClient, err := cfg.ComputeV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmt.Errorf("error creating compute client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_compute_eip_associate" {
			continue
		}
...
func testAccCheckVpcV1EIPExists(n string, eip *eips.PublicIp) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		networkingClient, err := config.NetworkingV1Client(acceptance.HCSO_REGION_NAME)
		if err != nil {
...
data "hcso_compute_instance" "test" {
  depends_on = [hcso_compute_eip_associate.test]
...
resource "hcso_compute_eip_associate" "test" {
  public_ip   = hcso_vpc_eip.test.address
  instance_id = hcso_compute_instance.test.id
  fixed_ip    = hcso_compute_instance.test.access_ip_v4
}
`, testAccComputeEIPAssociate_Base(rName))
}
...
func TestAccComputeEIPAssociate_bandwidth(t *testing.T) {
	...
	resourceName := "hcso_compute_eip_associate.test"
	bwResourceName := "hcso_vpc_bandwidth.bandwidth_1"
	ecsResourceName := "hcso_compute_instance.test"
...
data "hcso_images_image" "image_1" {
...
resource "hcso_compute_instance" "test" {
  name               = "%[1]s"
  image_id           = data.hcso_images_image.image_1.id
  availability_zone  = data.hcso_availability_zones.test.names[0]
...
resource "hcso_compute_eip_associate" "test" {
  bandwidth_id = hcso_vpc_bandwidth.bandwidth_1.id
  instance_id  = hcso_compute_instance.test.id
  fixed_ip     = hcso_compute_instance.test.access_ip_v6
}
`, rName)
}
