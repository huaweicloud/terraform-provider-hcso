...
func TestAccComputeInterfaceAttach_Basic(t *testing.T) {
	...
	resourceName := "hcso_compute_interface_attach.test"
...
func testAccCheckComputeInterfaceAttachDestroy(s *terraform.State) error {
	...
	computeClient, err := cfg.ComputeV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmt.Errorf("error creating compute client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_compute_interface_attach" {
			continue
		}
...
resource "hcso_vpc_subnet" "test" {
  ...
  name       = "%[1]s"
  cidr       = cidrsubnet(hcso_vpc.test.cidr, 4, 0)
  gateway_ip = cidrhost(cidrsubnet(hcso_vpc.test.cidr, 4, 0), 1)
}
...
resource "hcso_compute_interface_attach" "test" {
  ...
  fixed_ip           = cidrhost(cidrsubnet(hcso_vpc.test.cidr, 4, 0), 199)
  security_group_ids = [hcso_networking_secgroup.test.id]
}
`, rName)
}
