...
func testAccCheckMRSV2ClusterDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_mapreduce_cluster" {
			continue
		}
...
func testAccCheckMRSV2ClusterExists(n string, clusterGet *cluster.Cluster) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		mrsClient, err := config.MrsV1Client(acceptance.HCSO_REGION_NAME)
		if err != nil {
...
resource "hcso_mapreduce_cluster" "test" {
  ...
  external_datasources {
    ...
    source_type        = "RDS_MYSQL"
    data_connection_id = hcso_mapreduce_data_connection.test.id
  }
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "bootstrap_scripts.0.uri",
						acceptance.HCSO_MAPREDUCE_BOOTSTRAP_SCRIPT),
...
`, testAccMrsMapReduceClusterConfig_base(rName), rName, pwd, acceptance.HCSO_MAPREDUCE_BOOTSTRAP_SCRIPT)
}
