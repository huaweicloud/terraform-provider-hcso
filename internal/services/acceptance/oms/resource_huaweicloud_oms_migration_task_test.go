...
func TestAccMigrationTask_object(t *testing.T) {
	...
	smnName := rName + "-smn"
	resourceName := "hcso_oms_migration_task.test"
...
resource "hcso_obs_bucket_object" "source" {
...
`, baseConfig, smnName, acceptance.HCSO_REGION_NAME, acceptance.HCSO_ACCESS_KEY, acceptance.HCSO_SECRET_KEY)
}
...
resource "hcso_oms_migration_task" "test" {
  source_object {
    ...
    list_file_key    = hcso_obs_bucket_object.list_file_object.key
  }
