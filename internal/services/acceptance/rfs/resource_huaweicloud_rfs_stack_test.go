...
func getStackesourceFunc(config *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := config.AosV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
resource "hcso_obs_bucket_policy" "test" {
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "${hcso_obs_bucket.test.bucket}/rf/resource_stack/uri_test/*"
      ]
    }
  ]
}
...
resource "hcso_obs_bucket_object" "template" {
  bucket       = hcso_obs_bucket.test.bucket
...
resource "hcso_obs_bucket_object" "variable" {
  bucket       = hcso_obs_bucket.test.bucket
...
resource "hcso_rfs_stack" "test" {
  ...
  template_uri = "https://${hcso_obs_bucket.test.bucket_domain_name}/${hcso_obs_bucket_object.template.id}"
}
...
resource "hcso_rfs_stack" "test" {
  ...
  template_uri = "https://${hcso_obs_bucket.test.bucket_domain_name}/${hcso_obs_bucket_object.template.id}"
  vars_uri     = "https://${hcso_obs_bucket.test.bucket_domain_name}/${hcso_obs_bucket_object.variable.id}"
}
`, testAccStack_withUri_base(name, template, vars), name)
}
...
`, name, acceptance.HCSO_RF_TEMPLATE_ARCHIVE_NO_VARS_URI)
}
...
`, name, acceptance.HCSO_RF_TEMPLATE_ARCHIVE_URI, acceptance.HCSO_RF_VARIABLES_ARCHIVE_URI)
}
...
EOT
`, acceptance.HCSO_REGION_NAME, name)
}
