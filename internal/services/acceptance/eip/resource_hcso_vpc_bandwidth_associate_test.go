...
func TestAccBandWidthAssociate_basic(t *testing.T) {
	...
	resourceName := "hcso_vpc_bandwidth_associate.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "bandwidth_id", "hcso_vpc_bandwidth.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "eip_id", "hcso_vpc_eip.test.0", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "public_ip", "hcso_vpc_eip.test.0", "address"),
				),
			},
			{
				Config: testAccBandWidthAssociate_basic(randName, 1),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrPair(resourceName, "eip_id", "hcso_vpc_eip.test.1", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "public_ip", "hcso_vpc_eip.test.1", "address"),
				),
			},
			{
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "bandwidth_name", randName),
					resource.TestCheckResourceAttrPair(resourceName, "bandwidth_id", "hcso_vpc_bandwidth.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "eip_id", "hcso_vpc_eip.source", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "public_ip", "hcso_vpc_eip.source", "address"),
				),
			},
			{
				Config: testAccBandWidthAssociate_owner(randName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrPair(resourceName, "eip_id", "hcso_vpc_eip.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "public_ip", "hcso_vpc_eip.test", "address"),
				),
			},
		},
	})
...
resource "hcso_vpc_bandwidth" "source" {
  name = "%[1]s-source"
  size = 5
}

resource "hcso_vpc_eip" "source" {
  publicip {
    type = "5_bgp"
  }
  ...
  bandwidth {
    share_type = "WHOLE"
    id         = hcso_vpc_bandwidth.source.id
  }
