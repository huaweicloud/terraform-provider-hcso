		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "drill_vpc_id", "hcso_vpc.drill_vpc", "id"),
...
resource "hcso_vpc" "drill_vpc" {
  name = "%[2]s_drill_vpc"
  cidr = "192.168.0.0/16"
}

resource "hcso_vpc_subnet" "drill_vpc_subnet" {
  name       = "%[2]s_drill_vpc_subnet"
  vpc_id     = hcso_vpc.drill_vpc.id
  cidr       = "192.168.0.0/24"
...
resource "hcso_sdrs_drill" "test" {
  ...
  drill_vpc_id = hcso_vpc.drill_vpc.id

  depends_on = [
    hcso_sdrs_protected_instance.test,
    hcso_vpc_subnet.drill_vpc_subnet,
  ]
}
