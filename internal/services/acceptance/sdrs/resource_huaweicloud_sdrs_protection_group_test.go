...
func TestAccProtectionGroup_basic(t *testing.T) {
	...
	rName := "hcso_sdrs_protection_group.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "source_availability_zone", "data.hcso_availability_zones.test", "names.0"),
					resource.TestCheckResourceAttrPair(rName, "target_availability_zone", "data.hcso_availability_zones.test", "names.1"),
					resource.TestCheckResourceAttrPair(rName, "domain_id", "data.hcso_sdrs_domain.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "source_vpc_id", "hcso_vpc.test", "id"),
				),
			},
			{
...
resource "hcso_sdrs_protection_group" "test" {
  name                     = "%[2]s"
  source_availability_zone = data.hcso_availability_zones.test.names[0]
  target_availability_zone = data.hcso_availability_zones.test.names[1]
  domain_id                = data.hcso_sdrs_domain.test.id
  source_vpc_id            = hcso_vpc.test.id
