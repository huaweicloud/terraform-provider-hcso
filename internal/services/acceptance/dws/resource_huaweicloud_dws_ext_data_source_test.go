...
func TestAccDwsExtDataSource_basic(t *testing.T) {
	...
	rName := "hcso_dws_ext_data_source.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "cluster_id", "hcso_dws_cluster.test", "id"),
					resource.TestCheckResourceAttr(rName, "name", name),
					resource.TestCheckResourceAttr(rName, "type", "MRS"),
					resource.TestCheckResourceAttr(rName, "user_name", "admin"),
					resource.TestCheckResourceAttrPair(rName, "data_source_id", "hcso_mapreduce_cluster.test", "id"),
...
resource "hcso_dws_ext_data_source" "test" {
  cluster_id     = hcso_dws_cluster.test.id
  name           = "%s"
  type           = "MRS"
  data_source_id = hcso_mapreduce_cluster.test.id
  user_name      = "admin"
  user_pwd       = "%s"
