...
func TestAccDwsEventSubs_basic(t *testing.T) {
	...
	rName := "hcso_dws_event_subscription.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "notification_target", "hcso_smn_topic.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "notification_target_name", "hcso_smn_topic.test", "name"),
...
resource "hcso_dws_event_subscription" "test" {
  name                     = "%s"
  enable                   = 1
  notification_target      = hcso_smn_topic.test.id
  notification_target_type = "SMN"
  notification_target_name = hcso_smn_topic.test.name
