		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2FloatingIPExists("hcso_networking_floatingip_v2.fip_1", &fip),
				),
			},
			{
				ResourceName:      "hcso_networking_floatingip_v2.fip_1",
				ImportState:       true,
...
func testAccCheckNetworkingV2FloatingIPDestroy(s *terraform.State) error {
	...
	networkClient, err := config.NetworkingV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloud floating IP: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_networking_floatingip_v2" {
			continue
		}
...
resource "hcso_networking_floatingip_v2" "fip_1" {
}
`
...
resource "hcso_networking_port_v2" "port_1" {
  admin_state_up = "true"
  network_id = "${hcso_networking_subnet_v2.subnet_1.network_id}"
...
resource "hcso_networking_floatingip_v2" "fip_1" {
  ...
  fixed_ip = "${hcso_networking_port_v2.port_1.fixed_ip.0.ip_address}"
}
`, acceptance.HCSO_EXTGW_ID, acceptance.HCSO_POOL_NAME)
