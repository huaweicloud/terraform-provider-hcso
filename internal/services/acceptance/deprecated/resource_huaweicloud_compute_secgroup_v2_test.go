		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2SecGroupExists("hcso_compute_secgroup_v2.sg_1", &secgroup),
				),
			},
			{
				ResourceName:      "hcso_compute_secgroup_v2.sg_1",
				ImportState:       true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2SecGroupExists("hcso_compute_secgroup_v2.sg_1", &secgroup1),
					testAccCheckComputeV2SecGroupExists("hcso_compute_secgroup_v2.sg_2", &secgroup2),
					testAccCheckComputeV2SecGroupExists("hcso_compute_secgroup_v2.sg_3", &secgroup3),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"hcso_compute_secgroup_v2.sg_1", "rule.3170486100.self", "true"),
					resource.TestCheckResourceAttr(
						"hcso_compute_secgroup_v2.sg_1", "rule.3170486100.from_group_id", ""),
				),
			},
		},
	})
...
func testAccCheckComputeV2SecGroupDestroy(s *terraform.State) error {
	...
	computeClient, err := config.ComputeV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloud compute client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_compute_secgroup_v2" {
			continue
		}
...
resource "hcso_compute_secgroup_v2" "sg_1" {
  name = "sg_1"
...
resource "hcso_compute_secgroup_v2" "sg_2" {
  name = "sg_2"
...
resource "hcso_compute_secgroup_v2" "sg_3" {
  name = "sg_3"
  ...
  rule {
    ...
    to_port = 80
    ip_protocol = "tcp"
    from_group_id = "${hcso_compute_secgroup_v2.sg_1.id}"
  }
}
`
...
resource "hcso_compute_secgroup_v2" "sg_3" {
  ...
  rule {
    ...
    to_port = 80
    ip_protocol = "tcp"
    from_group_id = "${hcso_compute_secgroup_v2.sg_2.id}"
  }
}
`
