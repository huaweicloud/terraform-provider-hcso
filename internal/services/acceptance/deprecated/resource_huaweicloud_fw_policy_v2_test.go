		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWPolicyV2Exists(
						"hcso_fw_policy_v2.policy_1", "policy_1", "terraform acceptance test", 2),
				),
			},
			{
				ResourceName:      "hcso_fw_policy_v2.policy_1",
				ImportState:       true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWPolicyV2Exists(
						"hcso_fw_policy_v2.policy_1", "policy_1", "terraform acceptance test", 1),
				),
			},
		},
	})
...
resource "hcso_fw_policy_v2" "policy_1" {
  ...
  rules = [
    "${hcso_fw_rule_v2.udp_deny.id}",
    "${hcso_fw_rule_v2.tcp_allow.id}"
  ]
}

resource "hcso_fw_rule_v2" "tcp_allow" {
  protocol = "tcp"
  action = "allow"
}

resource "hcso_fw_rule_v2" "udp_deny" {
  protocol = "udp"
  action = "deny"
}
`
