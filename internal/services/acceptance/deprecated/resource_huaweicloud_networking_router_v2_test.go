...
func TestAccNetworkingV2Router_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "hcso_networking_router_v2.router_1",
				ImportState:       true,
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"hcso_networking_router_v2.router_1", "name", "router_2"),
				),
			},
		},
	})
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"hcso_networking_router_v2.router_1", "external_network_id", acceptance.HCSO_EXTGW_ID),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2RouterDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_networking_router_v2" {
			continue
		}
