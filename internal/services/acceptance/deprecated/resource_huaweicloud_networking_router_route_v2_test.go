		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2RouterExists("hcso_networking_router_v2.router_1", &router),
					testAccCheckNetworkingV2NetworkExists(
						"hcso_networking_network_v2.network_1", &network[0]),
					testAccCheckNetworkingV2SubnetExists(
						"hcso_networking_subnet_v2.subnet_1", &subnet[0]),
					testAccCheckNetworkingV2NetworkExists(
						"hcso_networking_network_v2.network_1", &network[1]),
					testAccCheckNetworkingV2SubnetExists(
						"hcso_networking_subnet_v2.subnet_1", &subnet[1]),
					testAccCheckNetworkingV2RouterInterfaceExists(
						"hcso_networking_router_interface_v2.int_1"),
					testAccCheckNetworkingV2RouterInterfaceExists(
						"hcso_networking_router_interface_v2.int_2"),
					testAccCheckNetworkingV2RouterRouteExists(
						"hcso_networking_router_route_v2.router_route_1"),
				),
			},
			{
				ResourceName:      "hcso_networking_router_route_v2.router_route_1",
				ImportState:       true,
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2RouterRouteExists(
						"hcso_networking_router_route_v2.router_route_1"),
					testAccCheckNetworkingV2RouterRouteExists(
						"hcso_networking_router_route_v2.router_route_2"),
				),
			},
			{
				Config: testAccNetworkingV2RouterRoute_destroy,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2RouterRouteEmpty("hcso_networking_router_v2.router_1"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2RouterRouteDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_networking_router_route_v2" {
			continue
		}
...
resource "hcso_networking_network_v2" "network_2" {
  name = "network_2"
  admin_state_up = "true"
}

resource "hcso_networking_subnet_v2" "subnet_2" {
  cidr = "192.168.200.0/24"
  ip_version = 4
  network_id = "${hcso_networking_network_v2.network_2.id}"
}
...
resource "hcso_networking_port_v2" "port_2" {
  ...
  fixed_ip {
    subnet_id = "${hcso_networking_subnet_v2.subnet_2.id}"
    ip_address = "192.168.200.1"
  }
}

resource "hcso_networking_router_interface_v2" "int_2" {
  router_id = "${hcso_networking_router_v2.router_1.id}"
...
resource "hcso_networking_router_route_v2" "router_route_2" {
  destination_cidr = "10.0.2.0/24"
  next_hop = "192.168.200.254"

  depends_on = ["hcso_networking_router_interface_v2.int_2"]
  router_id = "${hcso_networking_router_v2.router_1.id}"
}
`
