			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"hcso_networking_network_v2.network_1", "name", "network_2"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2RouterInterfaceExists(
						"hcso_networking_router_interface_v2.ri_1"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2NetworkDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_networking_network_v2" {
			continue
		}
...
resource "hcso_networking_router_interface_v2" "ri_1" {
