		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBlockStorageV2VolumeExists("hcso_blockstorage_volume_v2.volume_1", &volume),
					testAccCheckBlockStorageV2VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"hcso_blockstorage_volume_v2.volume_1", "name", "volume_1"),
					resource.TestCheckResourceAttr(
						"hcso_blockstorage_volume_v2.volume_1", "size", "1"),
				),
			},
			{
				ResourceName:      "hcso_blockstorage_volume_v2.volume_1",
				ImportState:       true,
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckBlockStorageV2VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"hcso_blockstorage_volume_v2.volume_1", "name", "volume_1-updated"),
					resource.TestCheckResourceAttr(
						"hcso_blockstorage_volume_v2.volume_1", "size", "2"),
				),
			},
		},
	})
...
func testAccCheckBlockStorageV2VolumeDestroy(s *terraform.State) error {
	...
	blockStorageClient, err := config.BlockStorageV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloud block storage client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_blockstorage_volume_v2" {
			continue
		}
...
resource "hcso_compute_instance_v2" "basic" {
  name            = "instance_1"
...
resource "hcso_compute_volume_attach_v2" "va_1" {
  instance_id = "${hcso_compute_instance_v2.basic.id}"
  volume_id   = "${hcso_blockstorage_volume_v2.volume_1.id}"
}
`, acceptance.HCSO_AVAILABILITY_ZONE, acceptance.HCSO_NETWORK_ID)
