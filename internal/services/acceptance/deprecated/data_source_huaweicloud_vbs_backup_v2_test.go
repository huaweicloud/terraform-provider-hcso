		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVBSBackupV2DataSourceID("data.hcso_vbs_backup_v2.backups"),
					resource.TestCheckResourceAttr("data.hcso_vbs_backup_v2.backups", "name", "vbs-backup"),
					resource.TestCheckResourceAttr("data.hcso_vbs_backup_v2.backups", "description", "Backup_Demo"),
				),
			},
		},
	})
...
resource "hcso_vbs_backup_v2" "backup_1" {
  volume_id = "${hcso_blockstorage_volume_v2.volume_1.id}"
  name = "vbs-backup"
  description = "Backup_Demo"
}

data "hcso_vbs_backup_v2" "backups" {
  id = "${hcso_vbs_backup_v2.backup_1.id}"
}
`
