...
func TestAccCTSTrackerV1DataSource_basic(t *testing.T) {
	dataName := "data.hcso_cts_tracker.tracker"
...
resource "hcso_cts_tracker" "tracker" {
  bucket_name               = hcso_obs_bucket.bucket.bucket
  file_prefix_name          = "yO8Q"
  is_support_smn            = true
  topic_id                  = hcso_smn_topic.topic_1.id
  is_send_all_key_operation = false
  operations                = ["login"]
  need_notify_user_list     = ["user1"]
}

data "hcso_cts_tracker" "tracker" {
  tracker_name = hcso_cts_tracker.tracker.id
}
`
