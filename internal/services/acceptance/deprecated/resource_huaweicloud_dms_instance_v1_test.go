...
func getDmsInstanceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.DmsV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccDmsInstancesV1_Rabbitmq(t *testing.T) {
	...
	resourceName := "hcso_dms_instance.instance_1"
...
resource "hcso_dms_instance" "instance_1" {
  ...
  product_id        = data.hcso_dms_product.product_1.id
  engine_version    = data.hcso_dms_product.product_1.version
  specification     = data.hcso_dms_product.product_1.bandwidth
  partition_num     = data.hcso_dms_product.product_1.partition_num
  storage_space     = data.hcso_dms_product.product_1.storage
  storage_spec_code = data.hcso_dms_product.product_1.storage_spec_code
...
}`, instanceName, acceptance.HCSO_VPC_ID, acceptance.HCSO_NETWORK_ID)
}
