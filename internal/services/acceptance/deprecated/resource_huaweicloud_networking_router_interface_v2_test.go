		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2RouterInterfaceExists("hcso_networking_router_interface_v2.int_1"),
				),
			},
			{
				ResourceName:      "hcso_networking_network_v2.network_1",
				ImportState:       true,
...
func testAccCheckNetworkingV2RouterInterfaceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_networking_router_interface_v2" {
			continue
		}
