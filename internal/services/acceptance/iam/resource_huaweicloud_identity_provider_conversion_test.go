...
func getProviderConversionFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.IAMNoVersionClient(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccIdentityProviderConversion_basic(t *testing.T) {
	...
	resourceName := "hcso_identity_provider_conversion.conversion"
...
resource "hcso_identity_provider_conversion" "conversion" {
  provider_id = hcso_identity_provider.provider_1.id
