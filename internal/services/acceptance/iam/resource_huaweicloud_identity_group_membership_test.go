...
func TestAccIdentityGroupMembership_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_identity_group_membership.membership_1"
	)
...
func testAccCheckIdentityGroupMembershipDestroy(s *terraform.State) error {
	...
	identityClient, err := cfg.IdentityV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmt.Errorf("error creating IAM client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_identity_group_membership" {
			continue
		}
...
resource "hcso_identity_group_membership" "membership_1" {
  ...
  users = [hcso_identity_user.user_1.id]
}
...
resource "hcso_identity_group_membership" "membership_1" {
  group = hcso_identity_group.group_1.id
  users = [hcso_identity_user.user_1.id, hcso_identity_user.user_2.id]
}
...
resource "hcso_identity_group_membership" "membership_1" {
  group = hcso_identity_group.group_1.id
  users = [hcso_identity_user.user_2.id]
}
`, groupName, userName, password)
}
