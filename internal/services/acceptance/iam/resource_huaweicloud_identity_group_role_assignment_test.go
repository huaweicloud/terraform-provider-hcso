...
func getIdentityGroupRoleAssignmentResourceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	identityClient, err := c.IdentityV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return nil, fmt.Errorf("error creating IAM v3 client: %s", err)
	}

	iamClient, err := c.IAMV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccIdentityGroupRoleAssignment_basic(t *testing.T) {
	...
	resourceName := "hcso_identity_group_role_assignment.test"
	var role roles.Role
...
// test whether the old resource hcso_identity_role_assignment works
func TestAccIdentityGroupRoleAssignment_old(t *testing.T) {
	rName := acceptance.RandomAccResourceName()
	resourceName := "hcso_identity_role_assignment.test"
	var role roles.Role
...
resource "hcso_identity_group_role_assignment" "test" {
  ...
  role_id   = hcso_identity_role.test.id
  domain_id = "%s"
}
`, testAccIdentityGroupRoleAssignment_base(rName), acceptance.HCSO_DOMAIN_ID)
}
...
`, testAccIdentityGroupRoleAssignment_base(rName), acceptance.HCSO_PROJECT_ID)
}
...
`, testAccIdentityGroupRoleAssignment_base(rName), acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
...
resource "hcso_identity_role_assignment" "test" {
  group_id  = hcso_identity_group.test.id
  role_id   = hcso_identity_role.test.id
  domain_id = "%s"
}
