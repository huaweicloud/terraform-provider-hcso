...
func TestAccCTSDataTracker_basic(t *testing.T) {
	...
	resourceName := "hcso_cts_data_tracker.tracker"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "data_bucket",
						"hcso_obs_bucket.data_bucket", "bucket"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "bucket_name",
						"hcso_obs_bucket.trans_bucket", "bucket"),
...
func testAccCTSDataTracker_basic(rName string) string {
	return fmt.Sprintf(`
resource "hcso_obs_bucket" "data_bucket" {
  bucket = "%[1]s-data"
  acl    = "public-read"
}
...
resource "hcso_cts_data_tracker" "tracker" {
  name        = "%[1]s"
  data_bucket = hcso_obs_bucket.data_bucket.bucket
  lts_enabled = true
}
`, rName)
}
...
resource "hcso_obs_bucket" "trans_bucket" {
  bucket        = "%[1]s-log"
  acl           = "private"
  force_destroy = true
...
resource "hcso_cts_data_tracker" "tracker" {
  name                 = "%[1]s"
  data_bucket          = hcso_obs_bucket.data_bucket.bucket
  bucket_name          = hcso_obs_bucket.trans_bucket.bucket
