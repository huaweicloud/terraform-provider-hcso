...
func TestAccDatasourceModels_basic(t *testing.T) {
	rName := "data.hcso_modelarts_models.name_filter"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "models.0.id", "hcso_modelarts_model.test1", "id"),
...
data "hcso_modelarts_models" "test" {
  depends_on = [hcso_modelarts_model.test1, hcso_modelarts_model.test2]
}

output "default_filter_is_useful" {
  value = length(data.hcso_modelarts_models.test.models) > 2
}

data "hcso_modelarts_models" "name_filter" {
  name        = hcso_modelarts_model.test1.name
  exact_match = "true"

  depends_on = [hcso_modelarts_model.test1, hcso_modelarts_model.test2]
}
output "name_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_models.name_filter.models[*].name : v == "%[2]s"])
}

data "hcso_modelarts_models" "version_filter" {
  version = hcso_modelarts_model.test1.version

  depends_on = [hcso_modelarts_model.test1, hcso_modelarts_model.test2]
}
output "version_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_models.version_filter.models[*].version : v == "0.0.1"])
}

data "hcso_modelarts_models" "status_filter" {
  status = "published"

  depends_on = [hcso_modelarts_model.test1, hcso_modelarts_model.test2]
}
output "status_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_models.status_filter.models[*].status : v == "published"])
}

data "hcso_modelarts_models" "description_filter" {
  description = hcso_modelarts_model.test2.description

  depends_on = [hcso_modelarts_model.test1, hcso_modelarts_model.test2]
}
output "description_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_models.description_filter.models[*].description : v == "%[3]s"])
}

data "hcso_modelarts_models" "workspace_id_filter" {
  workspace_id = hcso_modelarts_model.test1.workspace_id

  depends_on = [hcso_modelarts_model.test1, hcso_modelarts_model.test2]
}
output "workspace_id_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_models.workspace_id_filter.models[*].workspace_id : v == "0"])
}

data "hcso_modelarts_models" "model_type_filter" {
  model_type = hcso_modelarts_model.test1.model_type

  depends_on = [hcso_modelarts_model.test1, hcso_modelarts_model.test2]
}
output "model_type_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_models.model_type_filter.models[*].model_type : v == "TensorFlow"])
}

data "hcso_modelarts_models" "not_model_type_filter" {
  not_model_type = "TensorFlow"

  depends_on = [hcso_modelarts_model.test1, hcso_modelarts_model.test2]
}
output "not_model_type_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_models.not_model_type_filter.models[*].model_type : v != "TensorFlow"])
}
...
resource "hcso_modelarts_model" "test1" {
  name            = "%[1]s"
  version         = "0.0.1"
  description     = "This is a demo"
  source_location = "https://${hcso_obs_bucket.test.bucket_domain_name}/input"
...
resource "hcso_modelarts_model" "test2" {
  name            = "%[2]s"
  version         = "0.0.2"
  description     = "%[2]s"
  source_location = "https://${hcso_obs_bucket.test.bucket_domain_name}/input"
