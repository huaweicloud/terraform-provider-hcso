...
func TestAccDatasourceModelartServices_basic(t *testing.T) {
	rName := "data.hcso_modelarts_services.test"
...
func testAccDatasourceModelartServices_basic(name, name2 string) string {
	...
	return fmt.Sprintf(`
data "hcso_modelarts_models" "test" {
  description = "subscribe from market"
}

%[1]s

data "hcso_modelarts_services" "test" {
  depends_on = [
    hcso_modelarts_service.test,
    hcso_modelarts_service.test2
  ]
}

data "hcso_modelarts_services" "service_id_filter" {
  service_id = hcso_modelarts_service.test.id
...
output "service_id_filter_is_useful" {
  value = length(data.hcso_modelarts_services.service_id_filter.services) > 0 && alltrue(
    [for v in data.hcso_modelarts_services.service_id_filter.services[*].id : v == hcso_modelarts_service.test.id]
  )
}

data "hcso_modelarts_services" "name_filter" {
  name = "%[2]s"
  depends_on = [
    hcso_modelarts_service.test,
    hcso_modelarts_service.test2
...
output "name_filter_is_useful" {
  value = length(data.hcso_modelarts_services.name_filter.services) > 0 && alltrue(
    [for v in data.hcso_modelarts_services.name_filter.services[*].name : v == "%[2]s"]
  )
}

data "hcso_modelarts_services" "model_id_filter" {
  model_id = data.hcso_modelarts_models.test.models.0.id
  depends_on = [
    hcso_modelarts_service.test,
    hcso_modelarts_service.test2
  ]  
...
output "model_id_filter_is_useful" {
  value = length(data.hcso_modelarts_services.model_id_filter.services) > 0
}

data "hcso_modelarts_services" "infer_type_filter" {
...
output "infer_type_filter_is_useful" {
  value = length(data.hcso_modelarts_services.infer_type_filter.services) > 0 && alltrue(
    [for v in data.hcso_modelarts_services.infer_type_filter.services[*].infer_type : v == "real-time"]
  )
}

data "hcso_modelarts_services" "workspace_id_filter" {
  workspace_id = "0"
  depends_on = [
    hcso_modelarts_service.test,
    hcso_modelarts_service.test2
...
output "workspace_id_filter_is_useful" {
  value = length(data.hcso_modelarts_services.workspace_id_filter.services) > 0 && alltrue(
    [for v in data.hcso_modelarts_services.workspace_id_filter.services[*].workspace_id : v == "0"]
  )
}

data "hcso_modelarts_services" "status_filter" {
  status = hcso_modelarts_service.test.status
  depends_on = [
    hcso_modelarts_service.test,
    hcso_modelarts_service.test2
  ]
...
output "status_filter_is_useful" {
  value = length(data.hcso_modelarts_services.status_filter.services) > 0 && alltrue(
    [for v in data.hcso_modelarts_services.status_filter.services[*].status : v == hcso_modelarts_service.test.status]
  )
}
...
resource "hcso_smn_topic" "test2" {
  name         = "%[2]s"
  display_name = "This is a acc test"
}

resource "hcso_modelarts_service" "test2" {
  name        = "%[2]s"
  description = "This is a acc test"
  infer_type  = "real-time"
  ...
  config {
    ...
    instance_count = 1
    weight         = 100
    model_id       = data.hcso_modelarts_models.test.models.1.id
    envs = {
      "a" : "1",
      "b" : "2"
    }
  ...
  additional_properties {
    smn_notification {
      topic_urn = hcso_smn_topic.test2.id
      events    = [3]
    }
    log_report_channels {
      type = "LTS"
