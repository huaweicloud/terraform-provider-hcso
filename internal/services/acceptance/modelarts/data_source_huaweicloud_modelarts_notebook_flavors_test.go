...
func TestAccDatasourceNotebookFlavors_basic(t *testing.T) {
	rName := "data.hcso_modelarts_notebook_flavors.test"
...
func testAccDatasourceNotebookFlavors_basic() string {
	return `
data "hcso_modelarts_notebook_flavors" "test" {
}

data "hcso_modelarts_notebook_flavors" "category_filter" {
  category = "CPU"
}
output "category_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_notebook_flavors.category_filter.flavors[*].category : v == "CPU"])
}
`
}
