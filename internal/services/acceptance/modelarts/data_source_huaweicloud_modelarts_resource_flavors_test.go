...
func TestAccDatasourceResourceFlavors_basic(t *testing.T) {
	rName := "data.hcso_modelarts_resource_flavors.test"
...
func testAccDatasourceResourceFlavors_basic() string {
	return `
data "hcso_modelarts_resource_flavors" "test" {
}

data "hcso_modelarts_resource_flavors" "tag_filter" {
  tag = "os.modelarts/scope"
}
output "tag_filter_is_useful" {
  value = length(data.hcso_modelarts_resource_flavors.tag_filter.flavors) > 0 && length(
    data.hcso_modelarts_resource_flavors.tag_filter.flavors[*].tags["os.modelarts/scope"]
  ) == length(data.hcso_modelarts_resource_flavors.tag_filter.flavors)
}

data "hcso_modelarts_resource_flavors" "type_filter" {
  type = "Dedicate"
}
output "type_filter_is_useful" {
  value = length(data.hcso_modelarts_resource_flavors.type_filter.flavors) > 0 && alltrue(
    [for v in data.hcso_modelarts_resource_flavors.type_filter.flavors[*].type : v == "Dedicate"]
  )
}
`
}
