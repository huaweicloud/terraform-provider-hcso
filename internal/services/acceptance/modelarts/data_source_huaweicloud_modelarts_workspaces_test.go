...
func TestAccDatasourceWorkspaces_basic(t *testing.T) {
	rName := "data.hcso_modelarts_workspaces.name_filter"
...
resource "hcso_modelarts_workspace" "test2" {
  name        = "%[2]s"
  description = "%[2]s"
}

data "hcso_modelarts_workspaces" "name_filter" {
  name = "%[1]s"
  depends_on = [hcso_modelarts_workspace.test, hcso_modelarts_workspace.test2]
}
output "name_filter_is_useful" {
  value = length(data.hcso_modelarts_workspaces.name_filter.workspaces) > 0 && alltrue(
    [for v in data.hcso_modelarts_workspaces.name_filter.workspaces[*].name : v == "%[1]s"]
  )  
}

data "hcso_modelarts_workspaces" "enterprise_project_id_filter" {
  enterprise_project_id = "0"
  depends_on = [hcso_modelarts_workspace.test, hcso_modelarts_workspace.test2]
}
output "enterprise_project_id_filter_is_useful" {
  value = length(data.hcso_modelarts_workspaces.enterprise_project_id_filter.workspaces) > 0 && alltrue(
    [for v in data.hcso_modelarts_workspaces.enterprise_project_id_filter.workspaces[*].enterprise_project_id : v == "0"]
  )
}
`, name, name2)
}
