...
func TestAccDatasourceServiceFlavors_basic(t *testing.T) {
	rName := "data.hcso_modelarts_service_flavors.test"
...
func testAccDatasourceServiceFlavors_basic() string {
	return `
data "hcso_modelarts_service_flavors" "test" {
}

data "hcso_modelarts_service_flavors" "is_open_filter" {
  is_open = true
}
output "is_open_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_service_flavors.is_open_filter.flavors[*].is_open : v == true])
}

data "hcso_modelarts_service_flavors" "status_filter" {
  status = "normal"
}
output "status_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_service_flavors.status_filter.flavors[*].status : v == "normal"])
}

data "hcso_modelarts_service_flavors" "is_free_filter" {
  is_free = true
}
output "is_free_filter_is_useful" {
  value = alltrue([for v in data.hcso_modelarts_service_flavors.is_free_filter.flavors[*].is_free : v == true])
}
`
}
