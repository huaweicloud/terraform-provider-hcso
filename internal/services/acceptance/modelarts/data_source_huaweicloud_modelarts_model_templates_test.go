...
func TestAccDatasourceModelTemplates_basic(t *testing.T) {
	rName := "data.hcso_modelarts_model_templates.test"
...
data "hcso_modelarts_model_templates" "type_filter" {
  type = "Common"
}
output "type_filter_is_useful" {
  value = length(data.hcso_modelarts_model_templates.type_filter.templates) > 0 && alltrue(
    [for v in data.hcso_modelarts_model_templates.type_filter.templates[*].type : v == "Common"]
  )
}

data "hcso_modelarts_model_templates" "engine_filter" {
  engine = "TensorFlow1.8"
}
output "engine_filter_is_useful" {
  value = length(data.hcso_modelarts_model_templates.engine_filter.templates) > 0 && alltrue(
    [for v in data.hcso_modelarts_model_templates.engine_filter.templates[*].engine : v == "TensorFlow"]
  )
}

data "hcso_modelarts_model_templates" "environment_filter" {
  environment = "python3.6"
}
output "environment_filter_is_useful" {
  value = length(data.hcso_modelarts_model_templates.environment_filter.templates) > 0 && alltrue(
    [for v in data.hcso_modelarts_model_templates.environment_filter.templates[*].environment : strcontains(v, "python3.6")]
  )
}

data "hcso_modelarts_model_templates" "keyword_filter" {
  keyword = "CPU"
}
output "keyword_filter_is_useful" {
  value = length(data.hcso_modelarts_model_templates.keyword_filter.templates) > 0 && alltrue(
    [for v in data.hcso_modelarts_model_templates.keyword_filter.templates[*].description : strcontains(v, "CPU")]
  )
}
`
}
