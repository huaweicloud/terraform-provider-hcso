...
func TestAccASBandWidthPolicy_basic(t *testing.T) {
	...
	resourceName := "hcso_as_bandwidth_policy.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "status", "INSERVICE"),
					resource.TestCheckResourceAttrPair(resourceName, "bandwidth_id", "hcso_vpc_bandwidth.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "alarm_id", "hcso_ces_alarmrule.alarmrule_1", "id"),
				),
			},
			{
				ResourceName:      resourceName,
...
resource "hcso_ces_alarmrule" "alarmrule_1" {
  alarm_name           = "rule-%[1]s"
  ...
  metric {
    ...
    dimensions {
      name  = "bandwidth_id"
      value = hcso_vpc_bandwidth.test.id
    }
  }
...
resource "hcso_as_bandwidth_policy" "test" {
  ...
  bandwidth_id        = hcso_vpc_bandwidth.test.id
  alarm_id            = hcso_ces_alarmrule.alarmrule_1.id
