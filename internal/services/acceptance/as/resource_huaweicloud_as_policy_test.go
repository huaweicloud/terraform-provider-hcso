...
func TestAccASPolicy_basic(t *testing.T) {
	...
	resourceName := "hcso_as_policy.acc_as_policy"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "scaling_group_id", "hcso_as_group.acc_as_group", "id"),
				),
			},
			{
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "scaling_policy_action.0.instance_number", "1"),
					resource.TestCheckResourceAttrPair(resourceName, "scaling_group_id", "hcso_as_group.acc_as_group", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "alarm_id", "hcso_ces_alarmrule.alarm_rule", "id"),
				),
			},
			{
				ResourceName:      resourceName,
...
resource "hcso_as_policy" "acc_as_policy"{
...
resource "hcso_ces_alarmrule" "alarm_rule" {
  ...
  metric {
    ...
    dimensions {
      ...
      value = hcso_as_group.acc_as_group.id
    }
  }
  condition {
    period              = 300
