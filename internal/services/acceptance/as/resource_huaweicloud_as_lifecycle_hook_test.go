...
func TestAccASLifecycleHook_basic(t *testing.T) {
	...
	resourceGroupName := "hcso_as_group.acc_as_group"
	resourceHookName := "hcso_as_lifecycle_hook.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(resourceHookName, "notification_topic_urn",
						regexp.MustCompile(fmt.Sprintf(`^(urn:smn:%s:[0-9a-z]{32}:%s)$`, acceptance.HCSO_REGION_NAME, rName))),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(resourceHookName, "notification_topic_urn",
						regexp.MustCompile(fmt.Sprintf(`^(urn:smn:%s:[0-9a-z]{32}:%s-update)$`, acceptance.HCSO_REGION_NAME, rName))),
				),
			},
			{
...
func testAccCheckASLifecycleHookDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_as_lifecycle_hook" {
			continue
		}
...
resource "hcso_smn_topic" "update" {
  name = "%s-update"
}
...
resource "hcso_as_lifecycle_hook" "test" {
  name                   = "%s"
  type                   = "ADD"
  scaling_group_id       = hcso_as_group.acc_as_group.id
  notification_topic_urn = hcso_smn_topic.test.topic_urn
...
resource "hcso_as_lifecycle_hook" "test" {
  ...
  notification_topic_urn = hcso_smn_topic.update.topic_urn
