		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "floating_ip_id", "hcso_vpc_eip.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "private_ip", "hcso_compute_instance.test", "network.0.fixed_ip_v4"),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "nat_gateway_id", "hcso_nat_gateway.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "floating_ip_id", "hcso_vpc_eip.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "port_id", "hcso_compute_instance.test", "network.0.port"),
					resource.TestCheckResourceAttr(rName, "protocol", "udp"),
					resource.TestCheckResourceAttr(rName, "description", "Created by acc test"),
					resource.TestCheckResourceAttr(rName, "internal_service_port", "80"),
					resource.TestCheckResourceAttr(rName, "external_service_port", "8080"),
...
resource "hcso_nat_dnat_rule" "test" {
  ...
  floating_ip_id        = hcso_vpc_eip.test.id
  private_ip            = hcso_compute_instance.test.network[0].fixed_ip_v4
