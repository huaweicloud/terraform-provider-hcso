...
func getRuleCCProtectionResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	getRuleCCProtectionClient, err := cfg.NewServiceClient(getRuleCCProtectionProduct, acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccRuleCCProtection_basic(t *testing.T) {
	...
	rName := "hcso_waf_rule_cc_protection.test"
...
func TestAccRuleCCProtection_withEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testRuleCCProtection_basicWithEpsID(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttrPair(rName, "policy_id", "hcso_waf_policy.policy_1", "id"),
					resource.TestCheckResourceAttr(rName, "name", name),
