...
func TestAccDataSourceReferenceTablesV1_basic(t *testing.T) {
	...
	dataSourceName := "data.hcso_waf_reference_tables.ref_table"
...
func TestAccDataSourceReferenceTablesV1_withEpsID(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccReferenceTablesV1_conf_epsID(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckReferenceTablesId(dataSourceName),
...
data "hcso_waf_reference_tables" "ref_table" {
  depends_on = [hcso_waf_reference_table.ref_table]
}
