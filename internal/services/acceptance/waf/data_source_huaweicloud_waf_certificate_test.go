...
func TestAccDataSourceWafCertificateV1_basic(t *testing.T) {
	...
	dataSourceName := "data.hcso_waf_certificate.cert_1"
...
func TestAccDataSourceWafCertificateV1_withEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWafCertificateListV1_conf_withEpsID(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafCertDataSourceID(dataSourceName),
					resource.TestCheckResourceAttr(dataSourceName, "enterprise_project_id", acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
					resource.TestCheckResourceAttr(dataSourceName, "name", name),
					resource.TestCheckResourceAttr(dataSourceName, "expire_status", "1"),
					resource.TestCheckResourceAttrSet(dataSourceName, "expiration"),
				),
...
data "hcso_waf_certificate" "cert_1" {
  name          = hcso_waf_certificate.certificate_1.name
  expire_status = 1
