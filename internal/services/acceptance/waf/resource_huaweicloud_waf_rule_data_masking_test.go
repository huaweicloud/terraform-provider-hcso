...
func TestAccWafRuleDataMasking_basic(t *testing.T) {
	...
	resourceName1 := "hcso_waf_rule_data_masking.rule_1"
	resourceName2 := "hcso_waf_rule_data_masking.rule_2"
	resourceName3 := "hcso_waf_rule_data_masking.rule_3"
	resourceName4 := "hcso_waf_rule_data_masking.rule_4"
...
func TestAccWafRuleDataMasking_withEpsID(t *testing.T) {
	var rule rules.DataMasking
	policyName := acceptance.RandomAccResourceName()
	resourceName := "hcso_waf_rule_data_masking.rule"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWafRuleDataMasking_basic_withEpsID(policyName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
			...
			{
				Config: testAccWafRuleDataMasking_update_withEpsID(policyName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafRuleDataMaskingExists(resourceName, &rule),
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id", acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
...
func testAccCheckWafRuleDataMaskingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_waf_rule_data_masking" {
			continue
		}
...
resource "hcso_waf_rule_data_masking" "rule_2" {
  policy_id = hcso_waf_policy.policy_1.id
  path      = "/login"
  field     = "header"
  subfield  = "password"
}
resource "hcso_waf_rule_data_masking" "rule_3" {
  policy_id = hcso_waf_policy.policy_1.id
  path      = "/login"
  field     = "form"
  subfield  = "password"
}
resource "hcso_waf_rule_data_masking" "rule_4" {
  policy_id = hcso_waf_policy.policy_1.id
  path      = "/login"
  field     = "cookie"
  subfield  = "password"
...
resource "hcso_waf_rule_data_masking" "rule" {
  policy_id             = hcso_waf_policy.policy_1.id
  path                  = "/login"
  field                 = "params"
  subfield              = "password"
