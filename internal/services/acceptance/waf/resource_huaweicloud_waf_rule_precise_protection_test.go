...
func getRulePreciseProtectionResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	preciseProtectionClient, err := cfg.NewServiceClient(product, acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccRulePreciseProtection_basic(t *testing.T) {
	...
	rName := "hcso_waf_rule_precise_protection.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "policy_id", "hcso_waf_policy.policy_1", "id"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttrPair(rName, "policy_id", "hcso_waf_policy.policy_1", "id"),
					resource.TestCheckResourceAttrPair(rName, "conditions.1.reference_table_id", "hcso_waf_reference_table.ref_table", "id"),
...
func TestAccRulePreciseProtection_WithEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testRulePreciseProtection_basicWithEpsID(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
			...
			{
				Config: testRulePreciseProtection_updateWithEpsID(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(rName, "enterprise_project_id", acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
					resource.TestCheckResourceAttr(rName, "name", fmt.Sprintf("%s_update", name)),
					...
					resource.TestCheckResourceAttrPair(rName, "conditions.0.reference_table_id", "hcso_waf_reference_table.ref_table", "id"),
				),
			},
			{
				ResourceName:      rName,
...
resource "hcso_waf_rule_precise_protection" "test" {
  ...
  conditions {
    ...
    logic              = "prefix_any"
    subfield           = "test_sub"
    reference_table_id = hcso_waf_reference_table.ref_table.id
  }
}
