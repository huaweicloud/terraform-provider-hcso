...
func TestAccWafDedicatedInstance_basic(t *testing.T) {
	...
	resourceName := "hcso_waf_dedicated_instance.instance_1"
...
func TestAccWafDedicatedInstance_withEpsId(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWafDedicatedInstance_epsId(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
			...
			{
				Config: testAccWafDedicatedInstance_epsId(name, acceptance.HCSO_ENTERPRISE_MIGRATE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					acceptance.TestCheckResourceAttrWithVariable(resourceName, "group_id",
						"${hcso_waf_instance_group.group_1.id}"),
				),
			},
		},
	})
