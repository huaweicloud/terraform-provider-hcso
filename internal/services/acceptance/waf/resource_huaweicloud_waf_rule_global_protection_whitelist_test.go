...
func getRuleGlobalProtectionWhitelistResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	getClient, err := cfg.NewServiceClient(getProduct, acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccRuleGlobalProtectionWhitelist_basic(t *testing.T) {
	...
	rName := "hcso_waf_rule_global_protection_whitelist.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "domains.0", "hcso_waf_dedicated_domain.domain_1", "domain"),
...
func TestAccRuleGlobalProtectionWhitelist_withEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testRuleGlobalProtectionWhitelist_basicWithEpsID(randName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttrPair(rName, "policy_id", "hcso_waf_policy.policy_1", "id"),
					resource.TestCheckResourceAttrPair(rName, "domains.0", "hcso_waf_dedicated_domain.domain_1", "domain"),
...
resource "hcso_waf_rule_global_protection_whitelist" "test" {
  ...
  domains               = [hcso_waf_dedicated_domain.domain_1.domain]
