...
func TestAccRuleAntiCrawler_basic(t *testing.T) {
	...
	rName := "hcso_waf_rule_anti_crawler.test"
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "conditions.0.reference_table_id",
						"hcso_waf_reference_table.ref_table", "id"),
...
`, testAccWafPolicyV1_basic_withEpsID(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST), name,
		acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
