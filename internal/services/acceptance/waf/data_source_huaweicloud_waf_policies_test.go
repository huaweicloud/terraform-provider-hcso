...
func TestAccDataSourceWafPoliciesV1_basic(t *testing.T) {
	...
	dataSourceName := "data.hcso_waf_policies.policies_1"
...
func TestAccDataSourceWafPoliciesV1_withEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWafPoliciesV1_conf_epsID(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafPoliciesID(dataSourceName),
...
data "hcso_waf_policies" "policies_1" {
  name = hcso_waf_policy.policy_1.name
}
