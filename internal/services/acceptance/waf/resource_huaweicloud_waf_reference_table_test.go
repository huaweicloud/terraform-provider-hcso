...
func TestAccWafReferenceTableV1_basic(t *testing.T) {
	...
	resourceName := "hcso_waf_reference_table.ref_table"
...
func TestAccWafReferenceTableV1_withEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWafReferenceTableV1_conf_withEpsID(name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafReferenceTableV1Exists(resourceName, &referencTable),
			...
			{
				Config: testAccWafReferenceTableV1_update_withEpsID(updateName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafReferenceTableV1Exists(resourceName, &referencTable),
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id", acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
					resource.TestCheckResourceAttr(resourceName, "name", updateName),
...
func testAccCheckWafReferenceTableV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_waf_reference_table" {
			continue
		}
