...
func TestAccWafDedicateDomainV1_basic(t *testing.T) {
	...
	resourceName := "hcso_waf_dedicated_domain.domain_1"
...
func TestAccWafDedicateDomainV1_withEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWafDedicatedDomainV1_basic_withEpsID(randName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafDedicatedDomainV1Exists(resourceName, &domain),
			...
			{
				Config: testAccWafDedicatedDomainV1_update_withEpsID(randName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafDedicatedDomainV1Exists(resourceName, &domain),
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id", acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
					resource.TestCheckResourceAttr(resourceName, "proxy", "true"),
			...
			{
				Config: testAccWafDedicatedDomainV1_policy_withEpsID(randName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafDedicatedDomainV1Exists(resourceName, &domain),
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id", acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
					resource.TestCheckResourceAttr(resourceName, "domain", fmt.Sprintf("www.%s.com", randName)),
...
func testAccCheckWafDedicatedDomainV1Destroy(s *terraform.State) error {
	...
	c, err := config.WafDedicatedV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmt.Errorf("error creating HuaweiCloud WAF dedicated client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_waf_dedicated_domain" {
			continue
		}
...
resource "hcso_waf_dedicated_domain" "domain_1" {
  ...
  depends_on = [
    hcso_waf_certificate.certificate_1
  ]
}
