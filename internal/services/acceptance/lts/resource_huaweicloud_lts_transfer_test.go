		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "log_group_id", "hcso_lts_group.group", "id"),
					resource.TestCheckResourceAttrPair(rName, "log_streams.0.log_stream_id",
						"hcso_lts_stream.stream", "id"),
...
func testLtsTransfer_basic(name, obsName string) string {
	return fmt.Sprintf(`
resource "hcso_lts_group" "group" {
  group_name  = "%[1]s"
  ttl_in_days = 1
}
resource "hcso_lts_stream" "stream" {
  group_id    = hcso_lts_group.group.id
  stream_name = "%[1]s"
}
...
resource "hcso_lts_transfer" "test" {
  log_group_id = hcso_lts_group.group.id

  log_streams {
    log_stream_id = hcso_lts_stream.stream.id
  }
  ...
  log_transfer_info {
    ...
    log_transfer_detail {
      ...
      obs_bucket_name     = hcso_obs_bucket.output.bucket
...
resource "hcso_lts_transfer" "test" {
  ...
  log_transfer_info {
    ...
    log_transfer_detail {
      dis_id   = hcso_dis_stream.test.stream_id
      dis_name = hcso_dis_stream.test.stream_name
    }
  }
}
`, name, status)
...
func TestAccLtsTransfer_agency(t *testing.T) {
	...
	obsName := acceptance.RandomAccResourceNameWithDash()
	rName := "hcso_lts_transfer.test"
	rNameObs := "hcso_lts_transfer.test2"
...
resource "hcso_lts_stream" "stream_2" {
  group_id    = hcso_lts_group.group.id
  stream_name = "%[1]s_obs"
}
...
resource "hcso_lts_transfer" "test" {
  ...
  log_streams {
    log_stream_id = hcso_lts_stream.stream_1.id
  }
  ...
  log_transfer_info {
    ...
    log_agency_transfer {
      ...
      agency_domain_name = hcso_identity_agency.test.delegated_domain_name
      agency_name        = hcso_identity_agency.test.name
      agency_project_id  = "%[6]s"
    }
  }
}

resource "hcso_lts_transfer" "test2" {
  log_group_id = hcso_lts_group.group.id

  log_streams {
    log_stream_id = hcso_lts_stream.stream_2.id
  }
...
`, name, obsName, status, acceptance.HCSO_DOMAIN_NAME, acceptance.HCSO_DOMAIN_ID, acceptance.HCSO_PROJECT_ID)
}
