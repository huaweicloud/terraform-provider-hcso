...
func TestAccHostGroup_basic(t *testing.T) {
	...
	rName := "hcso_lts_host_group.test"
...
resource "null_resource" "test" {
  ...
  depends_on = [hcso_compute_instance.test]
}
`, name, acceptance.HCSO_ACCESS_KEY, acceptance.HCSO_SECRET_KEY)
}
...
output "is_host_id_different" {
  value = length(setsubtract(hcso_lts_host_group.test.host_ids,
    tolist([hcso_compute_instance.test[0].id]))) != 0
}
...
output "is_host_id_different" {
  value = length(setsubtract(hcso_lts_host_group.test.host_ids,
    hcso_compute_instance.test[*].id)) != 0
}
`, testHostGroup_base(name), name)
}
