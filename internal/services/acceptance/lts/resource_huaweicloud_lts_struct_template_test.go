...
func getLtsStructTemplateFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	c, _ := httpclient_go.NewHttpClientGo(conf, "lts", acceptance.HCSO_REGION_NAME)
...
func TestAccLtsStructTemplate_basic(t *testing.T) {
	...
	resourceName := "hcso_lts_struct_template.template_1"
...
func testAccLtsStructImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "hcso_lts_struct_template" {
...
func tesLtsStructTemplate_basic(rName string) string {
	return fmt.Sprintf(`
resource "hcso_lts_group" "group_1" {
  group_name  = "%[1]s"
  ttl_in_days = 1
}
resource "hcso_lts_stream" "stream_1" {
  group_id    = hcso_lts_group.group_1.id
  stream_name = "%[1]s"
}

resource "hcso_lts_struct_template" "template_1" {
  log_group_id  = hcso_lts_group.group_1.id
