...
func TestAccDatasourceCdmClusters_basic(t *testing.T) {
	rName := "data.hcso_cdm_clusters.test"
...
resource "hcso_cdm_cluster" "test2" {
...
data "hcso_cdm_clusters" "test" {
  depends_on = [
    hcso_cdm_cluster.test,
    hcso_cdm_cluster.test2
  ]
}

output "default_is_useful" {
  value = length(data.hcso_cdm_clusters.test.clusters) >= 2
}

data "hcso_cdm_clusters" "name_filter" {
  name = "%[3]s"
...
output "name_filter_is_useful" {
  value = length(data.hcso_cdm_clusters.name_filter.clusters) > 0 && alltrue(
    [for v in data.hcso_cdm_clusters.name_filter.clusters[*].name : v == "%[3]s"]
  )  
}

data "hcso_cdm_clusters" "az_filter" {
...
output "az_filter_is_useful" {
  value = length(data.hcso_cdm_clusters.az_filter.clusters) > 0 && alltrue(
    [for v in data.hcso_cdm_clusters.az_filter.clusters[*].availability_zone : v == data.hcso_availability_zones.test.names[0]]
  )  
}

data "hcso_cdm_clusters" "status_filter" {
  status = "200"
...
output "status_filter_is_useful" {
  value = length(data.hcso_cdm_clusters.status_filter.clusters) > 0 && alltrue(
    [for v in data.hcso_cdm_clusters.status_filter.clusters[*].status : v == "200"]
  )  
}

data "hcso_cdm_clusters" "error_status_filter" {
  status = "300"
...
output "error_status_filter_is_useful" {
  value = length(data.hcso_cdm_clusters.error_status_filter.clusters) == 0
}
