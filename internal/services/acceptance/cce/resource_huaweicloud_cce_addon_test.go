...
func getAddonFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.CceAddonV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccAddon_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_cce_addon.test"
...
resource "hcso_cce_addon" "test" {
  ...
  values {
    basic       = jsondecode(data.hcso_cce_addon_template.test.spec).basic
    custom_json = jsonencode(merge(
      jsondecode(data.hcso_cce_addon_template.test.spec).parameters.custom,
      {
        cluster_id = hcso_cce_cluster.test.id
        tenant_id  = "%[2]s"
        logLevel   = 3
    ...
    flavor_json = jsonencode(jsondecode(data.hcso_cce_addon_template.test.spec).parameters.flavor1)
  }
}
`, testAccAddon_values_base(name), acceptance.HCSO_PROJECT_ID)
}
...
resource "hcso_cce_addon" "test" {
  ...
  values {
    ...
    flavor_json = jsonencode(jsondecode(data.hcso_cce_addon_template.test.spec).parameters.flavor2)
  }
}
`, testAccAddon_values_base(rName), acceptance.HCSO_PROJECT_ID)
}
