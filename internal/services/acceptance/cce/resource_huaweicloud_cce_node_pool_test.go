...
func TestAccNodePool_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_cce_node_pool.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "security_groups.0",
						"hcso_networking_secgroup.test.0", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "security_groups.1",
						"hcso_networking_secgroup.test.1", "id"),
				),
			},
		},
	})
...
resource "hcso_networking_secgroup_rule" "rule1" {
  security_group_id = hcso_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
  remote_ip_prefix  = hcso_vpc_subnet.eni_test[0].cidr
}

resource "hcso_networking_secgroup_rule" "rule2" {
  security_group_id = hcso_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
  ports             = "10250"
  protocol          = "tcp"
  remote_ip_prefix  = hcso_vpc_subnet.test.cidr
}

resource "hcso_networking_secgroup_rule" "rule3" {
  security_group_id = hcso_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
...
resource "hcso_networking_secgroup_rule" "rule4" {
  security_group_id = hcso_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
...
resource "hcso_networking_secgroup_rule" "rule5" {
  security_group_id = hcso_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "egress"
  ethertype         = "IPv4"
...
resource "hcso_networking_secgroup_rule" "rule6" {
  security_group_id = hcso_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
  remote_group_id   = hcso_networking_secgroup.test[0].id
}

resource "hcso_networking_secgroup_rule" "rule7" {
  security_group_id = hcso_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
...
resource "hcso_cce_node_pool" "test" {
  ...
  pod_security_groups = [
    hcso_networking_secgroup.test[2].id,
    hcso_networking_secgroup.test[3].id
  ]
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "ecs_group_id",
						"hcso_compute_servergroup.test", "id"),
				),
			},
		},
	})
...
resource "hcso_cce_node_pool" "test" {
  ...
  priority                 = 0
  type                     = "vm"
  ecs_group_id             = hcso_compute_servergroup.test.id
