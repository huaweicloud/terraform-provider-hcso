		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "security_group_id",
						"hcso_networking_secgroup.test1", "id"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "status", "Available"),
					resource.TestCheckResourceAttrPair(resourceName, "security_group_id",
						"hcso_networking_secgroup.test2", "id"),
				),
			},
		},
	})
...
resource "hcso_cce_cluster" "test" {
  ...
  container_network_type = "overlay_l2"
  authentication_mode    = "rbac"
  eip                    = hcso_vpc_eip.update.address
}
`, common.TestVpc(rName), rName)
}
...
`, common.TestVpc(rName), rName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
...
resource "hcso_vpc_subnet" "eni_test" {
  count      = %[3]d

  name       = "%[2]s-eni-${count.index}"
  cidr       = cidrsubnet(hcso_vpc.test.cidr, 8, count.index + 1)
  gateway_ip = cidrhost(cidrsubnet(hcso_vpc.test.cidr, 8, count.index + 1), 1)
  vpc_id     = hcso_vpc.test.id
}
...
resource "hcso_cce_cluster" "test" {
  ...
  eni_subnet_id          = join(",", hcso_vpc_subnet.eni_test[*].ipv4_subnet_id)
}

output "is_eni_subnet_id_different" {
  value = length(setsubtract(split(",", hcso_cce_cluster.test.eni_subnet_id),
  hcso_vpc_subnet.eni_test[*].ipv4_subnet_id)) != 0
}
...
resource "hcso_networking_secgroup" "test1" {
  name = "%[2]s-secgroup-1"
}
...
resource "hcso_cce_cluster" "test" {
  ...
  security_group_id      = hcso_networking_secgroup.test1.id
...
resource "hcso_networking_secgroup" "test2" {
  name = "%[2]s-secgroup-2"
}
...
resource "hcso_cce_cluster" "test" {
  ...
  security_group_id      = hcso_networking_secgroup.test2.id
