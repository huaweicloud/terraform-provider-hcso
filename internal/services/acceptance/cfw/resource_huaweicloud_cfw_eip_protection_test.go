...
func getEipProtectionResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	client, err := cfg.NewServiceClient(getProtectedEipsProduct, acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccEipProtection_basic(t *testing.T) {
	var (
		...
		rName       = "hcso_cfw_eip_protection.test"
...
resource "hcso_cfw_eip_protection" "test" {
  ...
  dynamic "protected_eip" {
    for_each = slice(hcso_vpc_eip.test[*], 0, 2)
    content {
...
resource "hcso_cfw_eip_protection" "test" {
  ...
  dynamic "protected_eip" {
    for_each = slice(hcso_vpc_eip.test[*], 1, 3)
    content {
      id          = protected_eip.value.id
      public_ipv4 = protected_eip.value.address
    }
