...
func TestAccDatabaseUser_basic(t *testing.T) {
	...
	resourceName := "hcso_dds_database_user.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "roles.0.name",
						"hcso_dds_database_role.test", "name"),
					resource.TestCheckResourceAttrPair(resourceName, "inherited_privileges",
						"hcso_dds_database_role.test", "inherited_privileges"),
				),
			},
			{
...
func testAccDatabaseUserImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "hcso_dds_database_user" {
...
resource "hcso_dds_database_user" "test" {
  ...
  roles {
    name    = hcso_dds_database_role.test.name
    db_name = "admin"
  }
}
