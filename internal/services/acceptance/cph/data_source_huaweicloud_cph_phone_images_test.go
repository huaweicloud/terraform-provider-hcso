...
func TestAccDatasourcePhoneImages_basic(t *testing.T) {
	rName := "data.hcso_cph_phone_images.test"
...
func testAccDatasourcePhoneImages_basic() string {
	return `
data "hcso_cph_phone_images" "test" {
}

data "hcso_cph_phone_images" "is_public_filter" {
  is_public = 1
}
output "is_public_filter_is_useful" {
  value = alltrue([for v in data.hcso_cph_phone_images.is_public_filter.images[*].is_public : v == 1])
}

data "hcso_cph_phone_images" "image_label_filter" {
  image_label = "cloud_phone"
}
output "image_label_filter_is_useful" {
  value = alltrue([for v in data.hcso_cph_phone_images.image_label_filter.images[*].image_label : v == "cloud_phone"])
}
`
}
