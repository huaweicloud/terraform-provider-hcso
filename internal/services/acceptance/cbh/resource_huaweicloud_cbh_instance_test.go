			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "security_group_id",
						"data.hcso_networking_secgroup.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "public_ip_id",
						"hcso_vpc_eip.test_update", "id"),
					resource.TestCheckResourceAttrPair(rName, "public_ip",
						"hcso_vpc_eip.test_update", "address"),
				),
			},
			{
				ResourceName:            rName,
...
resource "hcso_cbh_instance" "test" {
  ...
  public_ip_id      = hcso_vpc_eip.test.id
...
`, testCBHInstance_base(), name, acceptance.HCSO_REGION_NAME)
}
...
resource "hcso_vpc_eip" "test_update" {
  publicip {
    type = "5_bgp"
  }
...
resource "hcso_cbh_instance" "test" {
  ...
  subnet_id         = data.hcso_vpc_subnet.test.id
  security_group_id = data.hcso_networking_secgroup.test.id
  public_ip_id      = hcso_vpc_eip.test_update.id
  public_ip         = hcso_vpc_eip.test_update.address
  availability_zone = data.hcso_availability_zones.test.names[0]
  region            = "%s"
