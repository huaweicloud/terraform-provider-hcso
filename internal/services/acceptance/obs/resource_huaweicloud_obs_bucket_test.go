...
func TestAccObsBucket_basic(t *testing.T) {
	...
	resourceName := "hcso_obs_bucket.bucket"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "region", acceptance.HCSO_REGION_NAME),
...
func TestAccObsBucket_withEpsId(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccObsBucket_epsId(rInt, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckObsBucketExists(resourceName),
					resource.TestCheckResourceAttr(
						resourceName, "bucket", testAccObsBucketName(rInt)),
					resource.TestCheckResourceAttr(
						resourceName, "enterprise_project_id", acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST),
				),
			},
			{
				Config: testAccObsBucket_epsId(rInt, acceptance.HCSO_ENTERPRISE_MIGRATE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckObsBucketExists(resourceName),
					resource.TestCheckResourceAttr(
						resourceName, "enterprise_project_id", acceptance.HCSO_ENTERPRISE_MIGRATE_PROJECT_ID_TEST),
				),
			},
			{
...
func testAccCheckObsBucketDestroy(s *terraform.State) error {
	...
	obsClient, err := conf.ObjectStorageClient(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating OBS client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_obs_bucket" {
			continue
		}
...
func testAccObsBucketDomainName(randInt int) string {
	return fmt.Sprintf("tf-test-bucket-%d.obs.%s.myhuaweicloud.com", randInt, acceptance.HCSO_REGION_NAME)
}
...
resource "hcso_obs_bucket" "bucket" {
  ...
  acl           = "private"
  encryption    = true
  kms_key_id    = hcso_kms_key.key_1.id
