...
func TestAccObsBucketReplication_basic(t *testing.T) {
	...
	rName := "hcso_obs_bucket_replication.replica"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccObsBucketReplication_basic(agencyName, bucketName, acceptance.HCSO_OBS_DESTINATION_BUCKET),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(rName, "bucket", bucketName),
					resource.TestCheckResourceAttr(rName, "agency", agencyName),
					resource.TestCheckResourceAttr(rName, "destination_bucket", acceptance.HCSO_OBS_DESTINATION_BUCKET),
			...
			{
				Config: testAccObsBucketReplication_update_1(agencyName, bucketName, acceptance.HCSO_OBS_DESTINATION_BUCKET),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
			...
			{
				Config: testAccObsBucketReplication_update_2(agencyName, bucketName, acceptance.HCSO_OBS_DESTINATION_BUCKET),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(rName, "rule.#", "1"),
...
resource "hcso_obs_bucket" "source" {
  bucket        = "%s"
...
resource "hcso_obs_bucket_replication" "replica" {
  bucket             = hcso_obs_bucket.source.bucket
  destination_bucket = "%s"
  agency             = hcso_identity_agency.agency.name
