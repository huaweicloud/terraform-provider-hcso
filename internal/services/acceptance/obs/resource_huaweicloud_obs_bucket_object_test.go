...
func TestAccObsBucketObject_source(t *testing.T) {
	...
	resourceName := "hcso_obs_bucket_object.object"
...
func testAccCheckObsBucketObjectDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_obs_bucket_object" {
			continue
		}
...
func testAccObsBucketObjecImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		bucket, ok := s.RootModule().Resources["hcso_obs_bucket.object_bucket"]
		if !ok {
			return "", fmt.Errorf("Bucket not found: %s", bucket)
		}
		object, ok := s.RootModule().Resources["hcso_obs_bucket_object.object"]
		if !ok {
...
func testAccObsBucketObjectConfigSource(randInt int, source string) string {
	return fmt.Sprintf(`
resource "hcso_obs_bucket" "object_bucket" {
  bucket = "tf-acc-test-bucket-%d"
}

resource "hcso_obs_bucket_object" "object" {
  bucket       = hcso_obs_bucket.object_bucket.bucket
  key          = "test-key"
  source       = "%s"
