...
func TestAccGaussDBAccountPrivilege_basic(t *testing.T) {
	...
	rName := "hcso_gaussdb_mysql_account_privilege.test"
...
resource "hcso_gaussdb_mysql_account_privilege" "test" {
  instance_id  = hcso_gaussdb_mysql_instance.test.id
  account_name = hcso_gaussdb_mysql_account.test.name
  host         = hcso_gaussdb_mysql_account.test.host

  databases {
    name     = hcso_gaussdb_mysql_database.test.name
    readonly = false
  }
}
