...
func getOpenGaussInstanceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := conf.OpenGaussV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccOpenGaussInstance_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_gaussdb_opengauss_instance.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "subnet_id", "hcso_vpc_subnet.test", "id"),
...
resource "hcso_networking_secgroup_rule" "in_v4_tcp_opengauss" {
...
resource "hcso_networking_secgroup_rule" "in_v4_tcp_opengauss_egress" {
  security_group_id = hcso_networking_secgroup.test.id
  ethertype         = "IPv4"
  direction         = "egress"
  protocol          = "tcp"
...
resource "hcso_gaussdb_opengauss_instance" "test" {
  ...
  coordinator_num   = 2
  replica_num       = %[4]d
  availability_zone = "${data.hcso_availability_zones.test.names[0]},${data.hcso_availability_zones.test.names[0]},${data.hcso_availability_zones.test.names[0]}"
