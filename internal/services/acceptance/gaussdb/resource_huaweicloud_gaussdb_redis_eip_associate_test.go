...
func TestAccGaussRedisEipAssociate_basic(t *testing.T) {
	...
	rName := "hcso_gaussdb_redis_eip_associate.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "instance_id",
						"hcso_gaussdb_redis_instance.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "node_id",
						"hcso_gaussdb_redis_instance.test", "nodes.0.id"),
...
resource "hcso_gaussdb_redis_eip_associate" "test" {
  instance_id = hcso_gaussdb_redis_instance.test.id
  node_id     = hcso_gaussdb_redis_instance.test.nodes[0].id
