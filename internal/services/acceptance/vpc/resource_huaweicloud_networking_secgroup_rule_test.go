...
func TestAccNetworkingSecGroupRule_basic(t *testing.T) {
	...
	resourceName := "hcso_networking_secgroup_rule.secgroup_rule_test"
...
func TestAccNetworkingSecGroupRule_noPorts(t *testing.T) {
	var secgroupRule rules.SecurityGroupRule
	resourceName := "hcso_networking_secgroup_rule.test"
	rName := acceptance.RandomAccResourceNameWithDash()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "remote_address_group_id",
						"hcso_vpc_address_group.test", "id"),
				),
			},
			{
				ResourceName:      resourceName,
...
func TestAccNetworkingSecGroupRule_action(t *testing.T) {
	var (
		...
		allowResName string = "hcso_networking_secgroup_rule.allow"
		denyResName  string = "hcso_networking_secgroup_rule.deny"
	)
...
func testAccNetworkingSecGroupRule_base(rName string) string {
	return fmt.Sprintf(`
resource "hcso_networking_secgroup" "secgroup_test" {
  name        = "%s-secgroup"
...
resource "hcso_networking_secgroup_rule" "secgroup_rule_test" {
  direction         = "ingress"
  ...
  security_group_id = hcso_networking_secgroup.secgroup_test.id
}
...
resource "hcso_networking_secgroup_rule" "secgroup_rule_test" {
  ...
  ports             = 80
  protocol          = "tcp"
  remote_group_id   = hcso_networking_secgroup.secgroup_test.id
  security_group_id = hcso_networking_secgroup.secgroup_test.id
}
`, testAccNetworkingSecGroupRule_base(rName))
}
...
resource "hcso_networking_secgroup_rule" "allow" {
  security_group_id = hcso_networking_secgroup.secgroup_test.id
  direction         = "ingress"
  ethertype         = "IPv4"
  ports             = 80
...
resource "hcso_networking_secgroup_rule" "deny" {
  security_group_id = hcso_networking_secgroup.secgroup_test.id
  direction         = "ingress"
  ethertype         = "IPv4"
  ports             = 8080
