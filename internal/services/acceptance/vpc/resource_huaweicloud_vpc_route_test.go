...
func getVpcRTBRouteResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	vpcClient, err := conf.NetworkingV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccVpcRTBRoute_basic(t *testing.T) {
	...
	resourceName := "hcso_vpc_route.test"
...
func TestAccVpcRTBRoute_vip(t *testing.T) {
	var route routetables.Route
	randName := acceptance.RandomAccResourceName()
	resourceName := "hcso_vpc_route.vip"
...
resource "hcso_vpc_route_table" "test" {
  name        = "%s"
  vpc_id      = hcso_vpc.test.id
  description = "a custom route table"
  subnets     = [hcso_vpc_subnet.test.id]
}
...
resource "hcso_vpc_route" "vip" {
  vpc_id         = hcso_vpc.test.id
  route_table_id = hcso_vpc_route_table.test.id
  destination    = "10.10.10.0/24"
  type           = "vip"
  nexthop        = hcso_networking_vip.test.ip_address
