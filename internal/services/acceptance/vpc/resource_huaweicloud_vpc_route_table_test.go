...
func TestAccVpcRouteTable_basic(t *testing.T) {
	...
	resourceName := "hcso_vpc_route_table.test"
...
resource "hcso_vpc_subnet" "test1-1" {
  name       = "%s-1-1"
  cidr       = "192.168.0.0/24"
...
resource "hcso_vpc_subnet" "test1-2" {
  name       = "%s-1-2"
...
resource "hcso_vpc_subnet" "test2-1" {
  name       = "%s-2-1"
  cidr       = "172.16.10.0/24"
  gateway_ip = "172.16.10.1"
  vpc_id     = hcso_vpc.test2.id
}
...
resource "hcso_vpc_route_table" "test" {
  ...
  subnets     = [
    hcso_vpc_subnet.test1-1.id,
    hcso_vpc_subnet.test1-2.id,
  ]
