...
// you shoule set `HCSO_CUSTOM_REGION_NAME` in your system and it should be different from `HCSO_REGION_NAME`.
func TestAccVpcV1_WithCustomRegion(t *testing.T) {
	...
	resName1 := "hcso_vpc.test1"
	resName2 := "hcso_vpc.test2"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpcV1_WithCustomRegion(vpcName1, vpcName2, acceptance.HCSO_CUSTOM_REGION_NAME),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCustomRegionVpcV1Exists(resName1, &vpc1, acceptance.HCSO_REGION_NAME),
					testAccCheckCustomRegionVpcV1Exists(resName2, &vpc2, acceptance.HCSO_CUSTOM_REGION_NAME),
				),
			},
		},
	})
...
func testAccCheckVpcV1Destroy(s *terraform.State) error {
	...
	vpcClient, err := config.NetworkingV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating internal vpc client: %s", err)
	}
