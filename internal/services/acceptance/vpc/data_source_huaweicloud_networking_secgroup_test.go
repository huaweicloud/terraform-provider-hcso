...
func TestAccNetworkingSecGroupV3DataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.hcso_networking_secgroup.test"
...
resource "hcso_networking_secgroup_rule" "ports" {
...
resource "hcso_networking_secgroup_rule" "port_range" {
  security_group_id = hcso_networking_secgroup.test.id
...
resource "hcso_networking_secgroup_rule" "remote_group" {
  security_group_id = hcso_networking_secgroup.test.id
...
resource "hcso_networking_secgroup_rule" "remote_address_group" {
  security_group_id = hcso_networking_secgroup.test.id
...
data "hcso_networking_secgroup" "test" {
  name = hcso_networking_secgroup.test.name
}
...
data "hcso_networking_secgroup" "test" {
  secgroup_id = hcso_networking_secgroup.test.id
}
`, testAccNetworkingSecGroupV3DataSource_group(rName))
}
