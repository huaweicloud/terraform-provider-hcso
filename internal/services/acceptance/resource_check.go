...
InitDataSourceCheck build a 'ResourceCheck' object. Only used to check datasource attributes.
	...
	Parameters:
	  dName: The data source name is used to check in the terraform.State. e.g. data.hcso_css_flavors.test
	Return:
...
InitResourceCheck build a 'ResourceCheck' object. The common test methods are provided in 'ResourceCheck'.
	...
	Parameters:
	  rName:           The resource name is used to check in the terraform.State. e.g. hcso_waf_domain.domain_1
...
TestCheckResourceAttrWithVariable validates the pair variable in state for the given name/key combination.
	...
	Parameters:
	  ...
	  pair such like ${hcso_waf_certificate.certificate_1.id}
	  or ${data.hcso_waf_policies.policies_2.policies.0.id}
*/
