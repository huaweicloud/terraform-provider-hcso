...
func TestAccDatasourceSmnMessageTemplates_basic(t *testing.T) {
	rName := "data.hcso_smn_message_templates.test"
...
data "hcso_smn_message_templates" "test" {
  depends_on = [hcso_smn_message_template.test]
}

data "hcso_smn_message_templates" "name_filter" {
  name = "%[2]s"
...
output "name_filter_is_useful" {
  value = length(data.hcso_smn_message_templates.name_filter.templates) > 0 && alltrue(
    [for v in data.hcso_smn_message_templates.name_filter.templates[*].name : v == "%[2]s"]
  )  
}

data "hcso_smn_message_templates" "protocol_filter" {
  protocol = "default"

  depends_on = [hcso_smn_message_template.test]
}
output "protocol_filter_is_useful" {
  value = length(data.hcso_smn_message_templates.protocol_filter.templates) > 0 && alltrue(
    [for v in data.hcso_smn_message_templates.protocol_filter.templates[*].protocol : v == "default"]
  )  
}

locals {
  template_id = hcso_smn_message_template.test.id
}

data "hcso_smn_message_templates" "template_id_filter" {
  template_id = local.template_id
}

output "template_id_filter_is_useful" {
  value = length(data.hcso_smn_message_templates.template_id_filter.templates) > 0 && alltrue(
    [for v in data.hcso_smn_message_templates.template_id_filter.templates[*].id : v == local.template_id]
  )  
}
