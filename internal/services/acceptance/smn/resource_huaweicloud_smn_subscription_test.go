...
func getResourceSMNSubscription(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	smnClient, err := conf.SmnV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccSMNV2Subscription_basic(t *testing.T) {
	...
	resourceName1 := "hcso_smn_subscription.subscription_1"
	resourceName2 := "hcso_smn_subscription.subscription_2"
	resourceName3 := "hcso_smn_subscription.subscription_3"
	resourceName4 := "hcso_smn_subscription.subscription_4"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(
						resourceName4, "endpoint", "hcso_fgs_function.test", "urn"),
				),
			},
			{
...
func testAccCheckSMNSubscriptionV2Destroy(s *terraform.State) error {
	...
	smnClient, err := cfg.SmnV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_smn_subscription" {
			continue
		}
...
resource "hcso_smn_subscription" "subscription_3" {
  topic_urn = hcso_smn_topic.topic_1.id
...
resource "hcso_smn_subscription" "subscription_4" {
  topic_urn = hcso_smn_topic.topic_1.id
  endpoint  = hcso_fgs_function.test.urn
