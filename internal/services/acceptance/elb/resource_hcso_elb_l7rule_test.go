...
func getELBl7RuleResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	lbClient, err := cfg.ElbV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccElbV3L7Rule_basic(t *testing.T) {
	...
	resourceName := "hcso_elb_l7rule.test"
...
resource "hcso_elb_l7rule" "test" {
  l7policy_id  = hcso_elb_l7policy.test.id
  type         = "PATH"
