...
func getELBl7PolicyResourceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	lbClient, err := c.ElbV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccElbV3L7Policy_basic(t *testing.T) {
	...
	resourceName := "hcso_elb_l7policy.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "redirect_listener_id",
						"hcso_elb_listener.test_redirect", "id"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "listener_id",
						"hcso_elb_listener.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "redirect_listener_id",
						"hcso_elb_listener.test_redirect_update", "id"),
				),
			},
			{
				ResourceName:      resourceName,
...
resource "hcso_elb_l7policy" "test" {
  name             = "%[2]s"
  description      = "test description"
  action           = "REDIRECT_TO_POOL"
  priority         = 20
  listener_id      = hcso_elb_listener.test.id
  redirect_pool_id = hcso_elb_pool.test.id
...
resource "hcso_elb_pool" "test_update" {
  name            = "%[2]s"
  protocol        = "HTTP"
  lb_method       = "LEAST_CONNECTIONS"
  loadbalancer_id = hcso_elb_loadbalancer.test.id
...
resource "hcso_elb_l7policy" "test" {
  ...
  priority         = 50
  listener_id      = hcso_elb_listener.test.id
  redirect_pool_id = hcso_elb_pool.test_update.id
...
resource "hcso_elb_listener" "test_redirect" {
  name                        = "%[3]s"
  protocol                    = "HTTPS"
  protocol_port               = 443
  loadbalancer_id             = hcso_elb_loadbalancer.test.id
  advanced_forwarding_enabled = true
  server_certificate          = hcso_elb_certificate.test.id
}
...
resource "hcso_elb_l7policy" "test" {
  ...
  listener_id          = hcso_elb_listener.test.id
  redirect_listener_id = hcso_elb_listener.test_redirect.id
}
...
resource "hcso_elb_listener" "test_redirect_update" {
  name                        = "%[4]s"
  protocol                    = "HTTPS"
  protocol_port               = 448
  loadbalancer_id             = hcso_elb_loadbalancer.test.id
...
resource "hcso_elb_l7policy" "test" {
  ...
  action               = "REDIRECT_TO_LISTENER"
  listener_id          = hcso_elb_listener.test.id
  redirect_listener_id = hcso_elb_listener.test_redirect_update.id
}
