...
func TestAccPolicyAssociate_basic(t *testing.T) {
	...
	rName := "hcso_cnad_advanced_policy_associate.test"
...
resource "hcso_cnad_advanced_policy_associate" "test" {
  policy_id            = hcso_cnad_advanced_policy.test.id
  instance_id          = data.hcso_cnad_advanced_instances.test.instances.0.instance_id
  protected_object_ids = ["%s"]
}
`, testPolicyAssociate_base(name), acceptance.HCSO_CNAD_PROJECT_OBJECT_ID)
}
