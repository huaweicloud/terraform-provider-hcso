...
func getDmsKafkaPermissionsFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.HcDmsV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccDmsKafkaPermissions_basic(t *testing.T) {
	...
	resourceName := "hcso_dms_kafka_permissions.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "policies.0.user_name",
						"hcso_dms_kafka_user.test1", "name"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "policies.1.user_name",
						"hcso_dms_kafka_user.test2", "name"),
...
resource "hcso_dms_kafka_user" "test1" {
...
resource "hcso_dms_kafka_permissions" "test" {
  instance_id = hcso_dms_kafka_instance.test.id
  topic_name  = hcso_dms_kafka_topic.topic.name

  policies {
    user_name     = hcso_dms_kafka_user.test1.name
    access_policy = "all"
  }
}
...
resource "hcso_dms_kafka_user" "test2" {
  instance_id = hcso_dms_kafka_instance.test.id
  name        = "%[2]s-2"
  password    = "%[3]s"
}
...
resource "hcso_dms_kafka_permissions" "test" {
  ...
  policies {
    user_name     = hcso_dms_kafka_user.test2.name
    access_policy = "sub"
  }
}
`, testAccDmsKafkaTopic_basic(rName), rName, password)
