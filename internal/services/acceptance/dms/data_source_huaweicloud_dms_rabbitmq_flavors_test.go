...
func TestAccRabbitMQFlavorsDataSource_basic(t *testing.T) {
	dataSourceName := "data.hcso_dms_rabbitmq_flavors.test"
...
func testAccDatasourceDmsRabbitMQFlavors_basic() string {
	return `
data "hcso_dms_rabbitmq_flavors" "baisc" {
  type = "cluster"
}
...
locals {
  test_refer   = data.hcso_dms_rabbitmq_flavors.baisc.flavors[0]
  test_results = data.hcso_dms_rabbitmq_flavors.test
}
