...
func TestAccKafkaInstance_basic(t *testing.T) {
	...
	resourceName := "hcso_dms_kafka_instance.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "tags.key", "value"),
					resource.TestCheckResourceAttr(resourceName, "tags.owner", "terraform"),
					resource.TestCheckResourceAttrPair(resourceName, "storage_space", "data.hcso_dms_product.test", "storage"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "broker_num",
						"data.hcso_dms_kafka_flavors.test", "flavors.0.properties.0.min_broker"),
					resource.TestCheckResourceAttrPair(resourceName, "flavor_id",
						"data.hcso_dms_kafka_flavors.test", "flavors.0.id"),
					resource.TestCheckResourceAttrPair(resourceName, "storage_spec_code",
						"data.hcso_dms_kafka_flavors.test", "flavors.0.ios.0.storage_spec_code"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "flavor_id",
						"data.hcso_dms_kafka_flavors.test", "flavors.1.id"),
					resource.TestCheckResourceAttrPair(resourceName, "storage_spec_code",
						"data.hcso_dms_kafka_flavors.test", "flavors.0.ios.0.storage_spec_code"),
...
resource "hcso_dms_kafka_instance" "test" {
  ...
  product_id        = data.hcso_dms_product.test.id
  engine_version    = data.hcso_dms_product.test.version
  storage_spec_code = data.hcso_dms_product.test.storage_spec_code
...
resource "hcso_dms_kafka_instance" "test" {
  ...
  engine_version    = data.hcso_dms_product.test.version
  storage_spec_code = data.hcso_dms_product.test.storage_spec_code
  storage_space     = data.hcso_dms_product.test.storage
  # use deprecated argument "bandwidth"
  bandwidth         = data.hcso_dms_product.test.bandwidth
...
locals {
  ...
  flavor    = data.hcso_dms_kafka_flavors.test.flavors[0]
  newFlavor = data.hcso_dms_kafka_flavors.test.flavors[1]
}
