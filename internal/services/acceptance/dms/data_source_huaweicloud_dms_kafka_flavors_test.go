...
func TestAccKafkaFlavorsDataSource_basic(t *testing.T) {
	dataSourceName := "data.hcso_dms_kafka_flavors.test"
...
data "hcso_dms_kafka_flavors" "baisc" {
  type = "cluster"
}
...
locals {
  test_refer   = data.hcso_dms_kafka_flavors.baisc.flavors[0]
  test_results = data.hcso_dms_kafka_flavors.test
}
