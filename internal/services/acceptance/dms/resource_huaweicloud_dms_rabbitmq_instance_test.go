...
func getDmsRabbitMqInstanceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.DmsV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccDmsRabbitmqInstances_basic(t *testing.T) {
	...
	resourceName := "hcso_dms_rabbitmq_instance.test"
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					acceptance.TestCheckResourceAttrWithVariable(resourceName, "product_id", "${data.hcso_dms_product.test2.id}"),
...
resource "hcso_dms_rabbitmq_instance" "test" {
  ...
  product_id        = data.hcso_dms_product.test1.id
  engine_version    = data.hcso_dms_product.test1.version
  storage_spec_code = data.hcso_dms_product.test1.storage_spec_code
...
resource "hcso_dms_rabbitmq_instance" "test" {
  ...
  product_id        = data.hcso_dms_product.test2.id
  engine_version    = data.hcso_dms_product.test2.version
  storage_spec_code = data.hcso_dms_product.test2.storage_spec_code
...
`, testAccDmsRabbitmqInstance_Base(rName), rName, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
...
resource "hcso_dms_rabbitmq_instance" "test" {
  ...
  network_id        = hcso_vpc_subnet.test.id
  security_group_id = hcso_networking_secgroup.test.id
  available_zones   = [data.hcso_dms_az.test.id]

  product_id        = data.hcso_dms_product.test1.id
  engine_version    = data.hcso_dms_product.test1.version
  storage_space     = data.hcso_dms_product.test1.storage
  storage_spec_code = data.hcso_dms_product.test1.storage_spec_code
...
resource "hcso_dms_rabbitmq_instance" "test" {
  ...
  product_id        = data.hcso_dms_product.single.id
  engine_version    = data.hcso_dms_product.single.version
  storage_spec_code = data.hcso_dms_product.single.storage_spec_code
  storage_space     = data.hcso_dms_product.single.storage
...
locals {
  query_results = data.hcso_dms_rabbitmq_flavors.test
  flavor        = data.hcso_dms_rabbitmq_flavors.test.flavors[0]
}
...
locals {
  query_results = data.hcso_dms_rabbitmq_flavors.test
  flavor        = data.hcso_dms_rabbitmq_flavors.test.flavors[0]
  newFlavor     = data.hcso_dms_rabbitmq_flavors.test.flavors[1]
}
