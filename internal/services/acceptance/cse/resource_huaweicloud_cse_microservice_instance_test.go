...
func TestAccMicroserviceInstance_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_cse_microservice_instance.test"
	)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "connect_address",
						"hcso_cse_microservice_engine.test", "service_registry_addresses.0.public"),
					resource.TestCheckResourceAttrPair(resourceName, "microservice_id",
						"hcso_cse_microservice.test", "id"),
					...
					resource.TestCheckResourceAttr(resourceName, "data_center.0.region", acceptance.HCSO_REGION_NAME),
...
func testAccMicroserviceInstanceImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "hcso_cse_microservice_instance" {
...
resource "hcso_cse_microservice" "test" {
  connect_address = hcso_cse_microservice_engine.test.service_registry_addresses.0.public
  ...
  admin_user = "root"
  admin_pass = hcso_cse_microservice_engine.test.admin_pass
}

resource "hcso_cse_microservice_instance" "test" {
  connect_address = hcso_cse_microservice_engine.test.service_registry_addresses.0.public

  microservice_id = hcso_cse_microservice.test.id
  host_name       = "localhost"
...
`, testAccMicroserviceEngine_config(rName), rName, acceptance.HCSO_REGION_NAME)
}
