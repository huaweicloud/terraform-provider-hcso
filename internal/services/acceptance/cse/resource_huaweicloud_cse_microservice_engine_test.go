...
func getEngineFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	c, err := conf.CseV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccMicroserviceEngine_basic(t *testing.T) {
	var (
		...
		resourceName = "hcso_cse_microservice_engine.test"
	)
...
}`, testAccMicroserviceEngine_base(name), name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
