...
func TestResourceCheck_parseVariable(t *testing.T) {
	// resource and data source within internal
	resource := "${hcso_vpc.test.id}"
	key, field, err := parseVariableToName(resource)
	AssertNoErr(t, err)
	AssertEquals(t, key, "hcso_vpc.test")
	AssertEquals(t, field, "id")

	data := "${data.hcso_compute_flavors.test.ids.0}"
	key, field, err = parseVariableToName(data)
	AssertNoErr(t, err)
	AssertEquals(t, key, "data.hcso_compute_flavors.test")
