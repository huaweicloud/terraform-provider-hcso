...
func TestAccTmsTag_basic(t *testing.T) {
	resourceName := "hcso_tms_tags.test"
...
func testAccCheckTmsTagDestroy(s *terraform.State) error {
	...
	client, err := conf.HcTmsV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
	...
	for key, value := range tags {
		...
		if len(tagsFromResponse) != 0 {
			return fmt.Errorf("hcso_tms_tags %s/%s still exists", key, value)
		}
	}
...
func testAccCheckTmsTagExists(key, value string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		return fmt.Errorf("hcso_tms_tags %s/%s does not exist", key, value)
	}
}
