...
func getSfsAccessRuleResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.SfsV2Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccSFSAccessRuleV2_basic(t *testing.T) {
	...
	resourceName := "hcso_sfs_access_rule.test"
...
resource "hcso_sfs_access_rule" "test" {
  sfs_id    = hcso_sfs_file_system.test.id
...
resource "hcso_sfs_access_rule" "test" {
  sfs_id    = hcso_sfs_file_system.test.id
  access_to = join("#", [data.hcso_vpc.test.id, "192.168.10.0/24", "0", "no_all_squash,no_root_squash"])
}`, rName)
}
