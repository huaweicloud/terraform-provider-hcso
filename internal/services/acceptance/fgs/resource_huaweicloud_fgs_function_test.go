...
func TestAccFgsV2Function_basic(t *testing.T) {
	...
	resourceName := "hcso_fgs_function.test"
...
func TestAccFgsV2Function_createByImage(t *testing.T) {
	var f function.Function
	randName := acceptance.RandomAccResourceName()
	rName1 := "hcso_fgs_function.create_with_vpc_access"
	rName2 := "hcso_fgs_function.create_without_vpc_access"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName1, "custom_image.0.url", acceptance.HCSO_BUILD_IMAGE_URL),
					resource.TestCheckResourceAttrPair(rName1, "vpc_id", "hcso_vpc.test", "id"),
					resource.TestCheckResourceAttrPair(rName1, "network_id", "hcso_vpc_subnet.test", "id"),
					...
					resource.TestCheckResourceAttr(rName2, "custom_image.0.url", acceptance.HCSO_BUILD_IMAGE_URL),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName1, "custom_image.0.url", acceptance.HCSO_BUILD_IMAGE_URL_UPDATED),
					rc2.CheckResourceExists(),
					resource.TestCheckResourceAttr(rName2, "handler", "-"),
					resource.TestCheckResourceAttrPair(rName2, "vpc_id", "hcso_vpc.test", "id"),
					resource.TestCheckResourceAttrPair(rName2, "network_id", "hcso_vpc_subnet.test", "id"),
					resource.TestCheckResourceAttr(rName2, "custom_image.0.url", acceptance.HCSO_BUILD_IMAGE_URL_UPDATED),
				),
			},
			{
				ResourceName:      rName1,
...
resource "hcso_fgs_function" "create_with_vpc_access" {
  name        = "%[2]s_1"
  app         = "default"
  handler     = "-"
  memory_size = 128
...
resource "hcso_fgs_function" "create_without_vpc_access" {
  name        = "%[2]s_2"
  app         = "default"
  handler     = "-"
  memory_size = 128
...
`, common.TestBaseNetwork(rName), rName, acceptance.HCSO_BUILD_IMAGE_URL)
}
...
`, common.TestBaseNetwork(rName), rName, acceptance.HCSO_BUILD_IMAGE_URL_UPDATED)
}
...
resource "hcso_fgs_function" "test" {
  ...
  network_id = hcso_vpc_subnet.test.id
  dns_list   = jsonencode(
    [for v in slice(hcso_dns_zone.test[*], 0, 2) : tomap({id=v.id, domain_name=v.name})]
  )
}
...
resource "hcso_fgs_function" "test" {
  ...
  network_id = hcso_vpc_subnet.test.id
  dns_list   = jsonencode(
    [for v in slice(hcso_dns_zone.test[*], 1, 3) : tomap({id=v.id, domain_name=v.name})]
  )
}
`, testAccFunction_domain_base(name), name)
}
