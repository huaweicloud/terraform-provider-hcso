...
func TestAccLBV2L7Rule_basic(t *testing.T) {
	...
	resourceName := "hcso_lb_l7rule.l7rule_1"
...
func testAccLBL7RuleImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		policy, ok := s.RootModule().Resources["hcso_lb_l7policy.l7policy_1"]
		if !ok {
			return "", fmt.Errorf("policy not found: %s", policy)
		}
		rule, ok := s.RootModule().Resources["hcso_lb_l7rule.l7rule_1"]
		if !ok {
