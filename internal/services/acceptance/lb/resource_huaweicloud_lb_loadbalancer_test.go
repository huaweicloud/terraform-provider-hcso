...
func getLoadBalancerResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	c, err := conf.LoadBalancerClient(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccLBV2LoadBalancer_basic(t *testing.T) {
	...
	resourceName := "hcso_lb_loadbalancer.loadbalancer_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV3SecGroupExists(
						"hcso_networking_secgroup.secgroup_1", &sg1),
					testAccCheckNetworkingV3SecGroupExists(
						"hcso_networking_secgroup.secgroup_1", &sg2),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV3SecGroupExists(
						"hcso_networking_secgroup.secgroup_2", &sg1),
					testAccCheckNetworkingV3SecGroupExists(
						"hcso_networking_secgroup.secgroup_2", &sg2),
					testAccCheckLBV2LoadBalancerHasSecGroup(&lb, &sg1),
...
func testAccCheckLBV2LoadBalancerHasSecGroup(
	...
	return func(s *terraform.State) error {
		...
		networkingClient, err := cfg.NetworkingV2Client(acceptance.HCSO_REGION_NAME)
		if err != nil {
...
func testAccCheckNetworkingV3SecGroupExists(n string, secGroup *groups.SecurityGroup) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		cfg := acceptance.TestAccProvider.Meta().(*config.Config)
		networkingClient, err := cfg.NetworkingV3Client(acceptance.HCSO_REGION_NAME)
		if err != nil {
...
resource "hcso_networking_secgroup" "secgroup_2" {
  name        = "%s"
...
resource "hcso_lb_loadbalancer" "loadbalancer_1" {
  ...
  security_group_ids = [
    hcso_networking_secgroup.secgroup_1.id
  ]
}
...
resource "hcso_lb_loadbalancer" "loadbalancer_1" {
  ...
  security_group_ids = [
    hcso_networking_secgroup.secgroup_1.id,
    hcso_networking_secgroup.secgroup_2.id
  ]
}
`, common.TestVpc(rName), rNameSg1, rNameSg2, rName)
}
