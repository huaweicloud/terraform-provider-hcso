...
func TestAccLBV2Member_basic(t *testing.T) {
	...
	resourceName1 := "hcso_lb_member.member_1"
	resourceName2 := "hcso_lb_member.member_2"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ExpectNonEmptyPlan: true, // Because admin_state_up remains false.
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("hcso_lb_member.member_1", "weight", "10"),
					resource.TestCheckResourceAttr("hcso_lb_member.member_2", "weight", "15"),
				),
			},
			{
				ResourceName:      "hcso_lb_member.member_1",
				ImportState:       true,
...
func testAccCheckLBV2MemberDestroy(s *terraform.State) error {
	...
	elbClient, err := config.LoadBalancerClient(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloud elb client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_lb_member" {
			continue
		}
...
func testAccLBMemberImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		pool, ok := s.RootModule().Resources["hcso_lb_pool.pool_1"]
		if !ok {
			return "", fmt.Errorf("pool not found: %s", pool)
		}
		member, ok := s.RootModule().Resources["hcso_lb_member.member_1"]
		if !ok {
...
resource "hcso_lb_loadbalancer" "loadbalancer_1" {
  name          = "%s"
  vip_subnet_id = data.hcso_vpc_subnet.test.ipv4_subnet_id
