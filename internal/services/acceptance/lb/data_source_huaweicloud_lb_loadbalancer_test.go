...
func TestAccELBV2LoadbalancerDataSource_basic(t *testing.T) {
	...
	dataSourceName1 := "data.hcso_lb_loadbalancer.test_by_name"
	dc1 := acceptance.InitDataSourceCheck(dataSourceName1)
	dataSourceName2 := "data.hcso_lb_loadbalancer.test_by_description"
	dc2 := acceptance.InitDataSourceCheck(dataSourceName2)

	var lb loadbalancers.LoadBalancer
	resourceName := "hcso_lb_loadbalancer.test"
...
data "hcso_lb_loadbalancer" "test_by_name" {
  name = hcso_lb_loadbalancer.test.name

  depends_on = [hcso_lb_loadbalancer.test]
}

data "hcso_lb_loadbalancer" "test_by_description" {
  description = hcso_lb_loadbalancer.test.description
