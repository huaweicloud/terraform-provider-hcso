...
func TestAccRdsDatabasePrivilege_basic(t *testing.T) {
	...
	rName := "hcso_rds_mysql_database_privilege.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "db_name",
						"hcso_rds_mysql_database.test", "name"),
					resource.TestCheckResourceAttrPair(rName, "users.0.name",
						"hcso_rds_mysql_account.test_1", "name"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "db_name",
						"hcso_rds_mysql_database.test", "name"),
					resource.TestCheckResourceAttrPair(rName, "users.0.name",
						"hcso_rds_mysql_account.test_2", "name"),
...
resource "hcso_rds_mysql_account" "test_1" {
...
resource "hcso_rds_mysql_database_privilege" "test" {
  ...
  users {
    name = hcso_rds_mysql_account.test_1.name
  }
}
...
resource "hcso_rds_mysql_account" "test_2" {
  instance_id = hcso_rds_instance.test.id
  name        = "%s_2"
  password    = "Test@12345678"
}
...
resource "hcso_rds_mysql_database_privilege" "test" {
  ...
  users {
    name     = hcso_rds_mysql_account.test_2.name
    readonly = true
  }
}
`, testMysqlDatabase_basic(rName, dbPwd, rName), rName)
