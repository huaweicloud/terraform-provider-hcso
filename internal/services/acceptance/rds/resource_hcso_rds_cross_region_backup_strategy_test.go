...
func TestAccBackupStrategy_basic(t *testing.T) {
	...
	rName := "hcso_rds_cross_region_backup_strategy.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "destination_region", acceptance.HCSO_DEST_REGION),
					resource.TestCheckResourceAttr(rName, "destination_project_id", acceptance.HCSO_DEST_PROJECT_ID),
				),
			},
			{
...
`, testAccRdsInstance_mysql_step1(name, dbPwd), acceptance.HCSO_DEST_REGION, acceptance.HCSO_DEST_PROJECT_ID)
}
