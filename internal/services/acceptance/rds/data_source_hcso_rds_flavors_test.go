...
func TestAccRdsFlavorDataSource_basic(t *testing.T) {
	dataSourceName := "data.hcso_rds_flavors.test"
...
data "hcso_rds_flavors" "base" {
  db_type = "MySQL"
}

data "hcso_rds_flavors" "test" {
  db_type    = "MySQL"
  group_type = data.hcso_rds_flavors.base.flavors[0].group_type
}

output "group_type_validation" {
  value = !contains([for a in data.hcso_rds_flavors.test.flavors[*].group_type : a == data.hcso_rds_flavors.base.flavors[0].group_type], false)
}
`
