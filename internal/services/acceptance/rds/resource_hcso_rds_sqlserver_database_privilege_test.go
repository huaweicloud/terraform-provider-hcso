...
func TestAccSQLServerDatabasePrivilege_basic(t *testing.T) {
	...
	rName := "hcso_rds_sqlserver_database_privilege.test"
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "users.#", "1"),
					resource.TestCheckResourceAttrPair(rName, "users.0.name",
						"hcso_rds_sqlserver_account.account_2", "name"),
...
resource "hcso_rds_sqlserver_account" "account_1" {
  instance_id = hcso_rds_instance.test.id
  name        = "%[2]s_1"
  password    = "Test@12345678"
}

resource "hcso_rds_sqlserver_database_privilege" "test" {
  instance_id = hcso_rds_instance.test.id
  db_name     = hcso_rds_sqlserver_database.test.name

  users {
    name = hcso_rds_sqlserver_account.account_1.name
  }
}
...
resource "hcso_rds_sqlserver_account" "account_2" {
  instance_id = hcso_rds_instance.test.id
  name        = "%[2]s_2"
  password    = "Test@12345678"
}
...
resource "hcso_rds_sqlserver_database_privilege" "test" {
  ...
  users {
    name     = hcso_rds_sqlserver_account.account_2.name
    readonly = true
  }
}
`, testSQLServerDatabase_basic(name), name)
