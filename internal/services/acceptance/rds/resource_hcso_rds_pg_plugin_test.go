...
func getPgPluginResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	getPgPluginClient, err := cfg.NewServiceClient("rds", acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccPgPlugin_basic(t *testing.T) {
	...
	resourceName := "hcso_rds_pg_plugin.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "instance_id", "hcso_rds_instance.test", "id"),
