...
func TestAccReadReplicaInstance_basic(t *testing.T) {
	...
	resourceType := "hcso_rds_read_replica_instance"
	resourceName := "hcso_rds_read_replica_instance.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "flavor",
						"data.hcso_rds_flavors.replica", "flavors.0.name"),
					resource.TestCheckResourceAttr(resourceName, "type", "Replica"),
...
resource "hcso_rds_read_replica_instance" "test" {
  name                = "%s"
  description         = "test_description"
  flavor              = data.hcso_rds_flavors.replica.flavors[0].name
  primary_instance_id = hcso_rds_instance.test.id
...
`, testAccRdsInstance_mysql_step1(name, dbPwd), name, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
