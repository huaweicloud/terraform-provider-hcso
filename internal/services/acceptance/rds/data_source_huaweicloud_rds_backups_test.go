...
func TestAccDatasourceBackup_basic(t *testing.T) {
	rName := "data.hcso_rds_backups.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "backups.0.id", "hcso_rds_backup.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "backups.0.name", "hcso_rds_backup.test", "name"),
