...
func getCssClusterFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := conf.CssV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccCssCluster_basic(t *testing.T) {
	...
	resourceName := "hcso_css_cluster.test"
...
resource "hcso_obs_bucket" "cssObs" {
  bucket        = "%s"
  acl           = "private"
  force_destroy = true
}
...
resource "hcso_css_cluster" "test" {
  name           = "%s"
  ...
  backup_strategy {
    ...
    prefix      = "snapshot"
    bucket      = hcso_obs_bucket.cssObs.bucket
