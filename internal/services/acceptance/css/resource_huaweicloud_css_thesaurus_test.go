...
func TestAccCssThesaurus_basic(t *testing.T) {
	...
	resourceName := "hcso_css_thesaurus.test"
...
resource "hcso_css_thesaurus" "test" {
  ...
  main_object = hcso_obs_bucket_object.test.key
}
...
func testAccCheckCssThesaurusExists(resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if !ok {
			return fmtp.Errorf("Error checking hcso_css_thesaurus exist, err=not found this resource")
		}

		resp, errQueryDetail := thesaurus.Get(client, rs.Primary.ID)
		if errQueryDetail != nil {
			return fmtp.Errorf("error checking hcso_css_thesaurus exist,err=send request failed:%s", errQueryDetail)
		}
