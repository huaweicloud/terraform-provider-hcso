...
func TestAccCssFlavorsDataSource_basic(t *testing.T) {
	var (
		typeFilter    = acceptance.InitDataSourceCheck("data.hcso_css_flavors.type_filter")
		versionFilter = acceptance.InitDataSourceCheck("data.hcso_css_flavors.version_filter")
		vcpusFilter   = acceptance.InitDataSourceCheck("data.hcso_css_flavors.vcpus_filter")
		memoryFilter  = acceptance.InitDataSourceCheck("data.hcso_css_flavors.memory_filter")
	)
...
data "hcso_css_flavors" "type_filter" {
  type = "ess"
}

output "is_type_filter_useful" {
  value = !contains([for v in data.hcso_css_flavors.type_filter.flavors[*].type : v == "ess"], "false")
}

data "hcso_css_flavors" "version_filter" {
  version = "7.9.3"
}

output "is_version_filter_useful" {
  value = !contains([for v in data.hcso_css_flavors.version_filter.flavors[*].version : v == "7.9.3"], "false")
}

data "hcso_css_flavors" "vcpus_filter" {
  vcpus = 32
}

output "is_vcpus_filter_useful" {
  value = !contains([for v in data.hcso_css_flavors.vcpus_filter.flavors[*].vcpus : v == 32], "false")
}

data "hcso_css_flavors" "memory_filter" {
  memory = 256
}

output "is_memory_filter_useful" {
  value = !contains([for v in data.hcso_css_flavors.memory_filter.flavors[*].memory : v == 256], "false")
}
`

func TestAccCssFlavorsDataSource_all(t *testing.T) {
	dataSourceName := "data.hcso_css_flavors.test"
