...
func getPolicyAssignmentResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.RmsV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
		return nil, fmt.Errorf("error creating RMS v1 client: %s", err)
	}

	return policyassignments.Get(client, acceptance.HCSO_DOMAIN_ID, state.Primary.ID)
}
...
func TestAccPolicyAssignment_basic(t *testing.T) {
	var (
		...
		rName       = "hcso_rms_policy_assignment.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "policy_definition_id",
						"data.hcso_rms_policy_definitions.test", "definitions.0.id"),
					resource.TestCheckResourceAttr(rName, "policy_filter.0.region", acceptance.HCSO_REGION_NAME),
...
resource "hcso_rms_policy_assignment" "test" {
  ...
  parameters = {
    listOfAllowedFlavors = "[\"${data.hcso_compute_flavors.test.ids[0]}\"]"
  }
}
`, basicConfig, name, status, acceptance.HCSO_REGION_NAME)
}
...
func testAccPolicyAssignment_periodConfig(name string) string {
	return fmt.Sprintf(`
resource "hcso_obs_bucket" "complian" {
  bucket        = "%[1]s"
...
resource "hcso_obs_bucket" "non_complian" {
...
resource "hcso_cts_tracker" "test" {
  bucket_name = hcso_obs_bucket.complian.bucket
...
resource "hcso_rms_policy_assignment" "test" {
  ...
  parameters = {
    trackBucket = "\"${hcso_obs_bucket.complian.bucket}\""
  }
}
...
resource "hcso_rms_policy_assignment" "test" {
  ...
  parameters = {
    trackBucket = "\"${hcso_obs_bucket.non_complian.bucket}\""
  }
}
`, periodConfig, name, status)
}
...
resource "hcso_rms_policy_assignment" "test" {
  ...
  custom_policy {
    function_urn = "${hcso_fgs_function.test.urn}:${hcso_fgs_function.test.version}"
    auth_type    = "agency"
    auth_value   = {
