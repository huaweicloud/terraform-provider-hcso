...
func TestAccKmsKeyDataSource_Basic(t *testing.T) {
	...
	datasourceName := "data.hcso_kms_key.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(datasourceName, "region", acceptance.HCSO_REGION_NAME),
				),
			},
		},
	})
...
func TestAccKmsKeyDataSource_WithTags(t *testing.T) {
	keyAlias := acceptance.RandomAccResourceName()
	var datasourceName = "data.hcso_kms_key.key_1"
	dc := acceptance.InitDataSourceCheck(datasourceName)
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:          func() { acceptance.TestAccPreCheckKms(t) },
		ProviderFactories: acceptance.TestAccProviderFactories,
...
data "hcso_kms_key" "key_1" {
  key_alias = hcso_kms_key.key_1.key_alias
  key_id    = hcso_kms_key.key_1.id
  key_state = "2"
}
...
data "hcso_kms_key" "key_1" {
  ...
  key_description       = "test description"
  key_state             = "2"
  enterprise_project_id = hcso_kms_key.key_1.enterprise_project_id
}
`, keyAlias, acceptance.HCSO_ENTERPRISE_PROJECT_ID_TEST)
}
