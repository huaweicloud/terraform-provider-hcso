...
func getKmsKeyResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := conf.KmsKeyV1Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccKmsKey_Basic(t *testing.T) {
	...
	var resourceName = "hcso_kms_key.key_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id",
						acceptance.HCSO_ENTERPRISE_PROJECT_ID),
				),
			},
		},
	})
...
`, keyAlias, acceptance.HCSO_REGION_NAME)
}
