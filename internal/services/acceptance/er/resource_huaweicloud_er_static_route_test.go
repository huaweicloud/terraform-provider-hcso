...
func getStaticRouteFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.ErV3Client(acceptance.HCSO_REGION_NAME)
	if err != nil {
...
func TestAccStaticRoute_basic(t *testing.T) {
	var (
		...
		sourceSelfResName = "hcso_er_static_route.source_self"
		destSelfResName   = "hcso_er_static_route.destination_self"
		crossVpcResName   = "hcso_er_static_route.cross_vpc"
...
resource "hcso_vpc" "source" {
  name = "%[1]s_source"
  cidr = cidrsubnet(var.base_vpc_cidr, 2, 1)
}

resource "hcso_vpc" "destination" {
  name = "%[1]s_destination"
  cidr = cidrsubnet(var.base_vpc_cidr, 2, 2)
}

resource "hcso_vpc_subnet" "source" {
  vpc_id = hcso_vpc.source.id

  name       = "%[1]s_source"
  cidr       = cidrsubnet(hcso_vpc.source.cidr, 2, 1)
  gateway_ip = cidrhost(cidrsubnet(hcso_vpc.source.cidr, 2, 1), 1)
}

resource "hcso_vpc_subnet" "destination" {
  vpc_id = hcso_vpc.destination.id

  name       = "%[1]s_destination"
  cidr       = cidrsubnet(hcso_vpc.destination.cidr, 2, 1)
  gateway_ip = cidrhost(cidrsubnet(hcso_vpc.destination.cidr, 2, 1), 1)
}
...
resource "hcso_er_route_table" "source" {
  instance_id = hcso_er_instance.test.id
  name        = "%[1]s_source"
}

resource "hcso_er_route_table" "destination" {
  instance_id = hcso_er_instance.test.id
  name        = "%[1]s_destination"
}

resource "hcso_er_vpc_attachment" "source" {
  instance_id = hcso_er_instance.test.id
  vpc_id      = hcso_vpc.source.id
  subnet_id   = hcso_vpc_subnet.source.id
  name        = "%[1]s_source"
}

resource "hcso_er_vpc_attachment" "destination" {
  instance_id = hcso_er_instance.test.id
  vpc_id      = hcso_vpc.destination.id
  subnet_id   = hcso_vpc_subnet.destination.id
  name        = "%[1]s_destination"
}
`, name, bgpAsNum)
}
...
resource "hcso_er_static_route" "source_self" {
  route_table_id = hcso_er_route_table.source.id
  destination    = hcso_vpc.source.cidr
  attachment_id  = hcso_er_vpc_attachment.source.id
}

resource "hcso_er_static_route" "destination_self" {
  route_table_id = hcso_er_route_table.destination.id
  destination    = hcso_vpc.destination.cidr
  attachment_id  = hcso_er_vpc_attachment.destination.id
}

resource "hcso_er_static_route" "cross_vpc" {
  route_table_id = hcso_er_route_table.source.id
  destination    = hcso_vpc.destination.cidr
  attachment_id  = hcso_er_vpc_attachment.source.id
}
