...
func TestAccRouteTablesDataSource_basic(t *testing.T) {
	var (
		dName    = "data.hcso_er_route_tables.test"
...
resource "hcso_er_route_table" "another" {
  instance_id = hcso_er_instance.test.id
  name        = "%[2]s_another"
...
output "route_tables_count" {
  value = length(data.hcso_er_route_tables.test.route_tables)
}
`, testAccRouteTablesDataSource_base(name, bgpAsNum), name)
}
