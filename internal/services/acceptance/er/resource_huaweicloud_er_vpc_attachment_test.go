...
func TestAccVpcAttachment_basic(t *testing.T) {
	var (
		...
		rName      = "hcso_er_vpc_attachment.test"
...
func testAccVpcAttachmentImportStateFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "hcso_er_vpc_attachment" {
...
data "hcso_vpc_route_table" "test" {
  depends_on = [
...
output "er_route_count" {
  value = length([for route in data.hcso_vpc_route_table.test.route : route.type == "er"])
}
