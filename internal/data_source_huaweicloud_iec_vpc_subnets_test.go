...
func TestAccIECSubnetsDataSource_basic(t *testing.T) {
	...
	allSubnets := "data.hcso_iec_vpc_subnets.all"
	siteSubnets := "data.hcso_iec_vpc_subnets.site"
...
resource "hcso_iec_vpc_subnet" "subnet_1" {
  name       = "%s-1"
  cidr       = "192.168.0.0/24"
...
resource "hcso_iec_vpc_subnet" "subnet_2" {
  name       = "%s-2"
  cidr       = "192.168.1.0/24"
  vpc_id     = hcso_iec_vpc.vpc_test.id
  site_id    = data.hcso_iec_sites.sites_test.sites[1].id
...
data "hcso_iec_vpc_subnets" "all" {
  vpc_id = hcso_iec_vpc.vpc_test.id
}

data "hcso_iec_vpc_subnets" "site" {
  vpc_id  = hcso_iec_vpc.vpc_test.id
  site_id = data.hcso_iec_sites.sites_test.sites[0].id
}
