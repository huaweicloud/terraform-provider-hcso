...
func TestAccBCSV2Instance_basic(t *testing.T) {
	...
	resourceName := "hcso_bcs_instance.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id", HCSO_ENTERPRISE_PROJECT_ID_TEST),
...
func testAccCheckBCSInstanceV2Destroy() resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		client, err := config.BcsV2Client(HCSO_REGION_NAME)
		if err != nil {
			return fmtp.Errorf("Error creating internal BCS client: %s", err)
		}

		for _, rs := range s.RootModule().Resources {
			if rs.Type != "hcso_bcs_instance" {
				continue
			}
...
resource "hcso_bcs_instance" "test" {
  ...
  name                  = "%s"
  cce_cluster_id        = hcso_cce_cluster.test.id
  consensus             = "etcdraft"
  edition               = 4
...
`, testBCSInstanceV2_base(rName), rName, HCSO_ENTERPRISE_PROJECT_ID_TEST, password)
}
...
`, testBCSInstanceV2_base(rName), rName, HCSO_ENTERPRISE_PROJECT_ID_TEST, password, password)
}
