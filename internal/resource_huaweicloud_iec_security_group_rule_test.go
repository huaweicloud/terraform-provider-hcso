...
func TestAccIecSecurityGroupRuleResource_Basic(t *testing.T) {
	...
	groupName := "hcso_iec_security_group.my_group"
	ruleName1 := "hcso_iec_security_group_rule.rule_1"
	ruleName2 := "hcso_iec_security_group_rule.rule_2"
...
func testAccCheckIecSecurityGroupRuleV1Destory(state *terraform.State) error {
	...
	for _, rs := range state.RootModule().Resources {
		if rs.Type != "hcso_iec_security_group_rule" {
			continue
		}
...
func testAccIecSecurityGroupRuleV1_Basic(rName string) string {
	return fmt.Sprintf(`
resource "hcso_iec_security_group" "my_group" {
  name = "%s"
}

resource "hcso_iec_security_group_rule" "rule_1" {
  direction = "egress"
  port_range_min = 445
  port_range_max = 445
  protocol = "tcp" 
  security_group_id = hcso_iec_security_group.my_group.id
  remote_ip_prefix = "0.0.0.0/0"
}

resource "hcso_iec_security_group_rule" "rule_2" {
  direction = "ingress"
  port_range_min = "20"
  port_range_max = "20"
  protocol = "udp" 
