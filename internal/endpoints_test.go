...
func testAccPreCheckServiceEndpoints(t *testing.T) {
	...
	projectID := os.Getenv("HCSO_PROJECT_ID")
	if projectID == "" {
		t.Fatalf(yellow("HCSO_PROJECT_ID must be set for service endpoint acceptance test"))
	}
}
...
func TestAccServiceEndpoints_IAM(t *testing.T) {
	...
	serviceClient, err = cfg.IAMV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud IAM client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://iam.%s.%s/v3.0/", HCSO_REGION_NAME, cfg.Cloud)
	...
	serviceClient, err = cfg.IdentityV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud identity client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://iam.%s.%s/v3/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.IAMNoVersionClient(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud IAM client without version number: %s", err)
	}
	expectedURL = fmt.Sprintf("https://iam.%s.%s/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
		t.Fatalf("Identity endpoint: expected %s but got %s", green(expectedURL), yellow(actualURL))
	}
...
func TestAccServiceEndpoints_Global(t *testing.T) {
	...
	serviceClient, err = cfg.CdnV1Client(HCSO_REGION_NAME)
	if err != nil {
	...
	serviceClient, err = cfg.BssV1Client(HCSO_REGION_NAME)
	if err != nil {
	...
	serviceClient, err = cfg.BssV2Client(HCSO_REGION_NAME)
	if err != nil {
	...
	serviceClient, err = cfg.EnterpriseProjectClient(HCSO_REGION_NAME)
	if err != nil {
	...
	serviceClient, err = cfg.RmsV1Client(HCSO_REGION_NAME)
	if err != nil {
...
func TestAccServiceEndpoints_Management(t *testing.T) {
	...
	serviceClient, err = cfg.CtsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud CTS client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cts.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.LtsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud LTS client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://lts.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.CesV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud CES client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://ces.%s.%s/V1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
...
func TestAccServiceEndpoints_Database(t *testing.T) {
	...
	serviceClient, err = cfg.RdsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud RDS v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://rds.%s.%s/rds/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.RdsV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud RDS v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://rds.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DdsV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud DDS v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dds.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.GeminiDBV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud GeminiDB client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://gaussdb-nosql.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.GeminiDBV31Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud GeminiDBV31 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://gaussdb-nosql.%s.%s/v3.1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.GaussdbV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud gaussdb client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://gaussdb.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.OpenGaussV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud openGauss client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://gaussdb-opengauss.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DrsV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud DRS client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://drs.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
...
func TestAccServiceEndpoints_Security(t *testing.T) {
	...
	serviceClient, err = cfg.AntiDDosV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud anti-ddos client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://antiddos.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.KmsKeyV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud KMS(v1.0) client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://kms.%s.%s/v1.0/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.KmsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud KMS(v1) client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://kms.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.KmsV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud KMS(v3) client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://kms.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.ScmV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud SCM client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://scm.%s.%s/v3/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.WafV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud WAF client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://waf.%s.%s/v1/%s/waf/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.WafDedicatedV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud WAF dedicated client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://waf.%s.%s/v1/%s/premium-waf/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
...
func TestAccServiceEndpoints_Application(t *testing.T) {
	...
	serviceClient, err = cfg.ApiGatewayV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud API-GW client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://apig.%s.%s/v1.0/apigw/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.ApigV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud API-GW v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://apig.%s.%s/v2/%s/apigw/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.BcsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud BCS v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://bcs.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.CseV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud CSE v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cse.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DcsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud dcs v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dcs.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DcsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud dcs v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dcs.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DmsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud DMS v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dms.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DmsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud DMS v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dms.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.ServiceStageV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud ServiceStage v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://servicestage.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	expectedURL = fmt.Sprintf("https://servicestage.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.EgV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("error creating EG v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://eg.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
...
func TestAccServiceEndpoints_Compute(t *testing.T) {
	...
	serviceClient, err = cfg.ComputeV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud ecs v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://ecs.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "ecs", "v1", t)

	// test for computeV11Client
	serviceClient, err = cfg.ComputeV11Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud ecs v1.1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://ecs.%s.%s/v1.1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "ecs", "v1.1", t)

	// test for computeV2Client
	serviceClient, err = cfg.ComputeV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud ecs v2.1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://ecs.%s.%s/v2.1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "ecs", "v2.1", t)

	// test for autoscalingV1Client
	serviceClient, err = cfg.AutoscalingV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud autoscaling v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://as.%s.%s/autoscaling-api/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "autoscaling", "v1", t)

	// test for imageV2Client
	serviceClient, err = cfg.ImageV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud image v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://ims.%s.%s/v2/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "image", "v2", t)

	// test for cceV3Client
	serviceClient, err = cfg.CceV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud cce v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cce.%s.%s/api/v3/projects/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "cce", "v3", t)

	// test for cceAddonV3Client
	serviceClient, err = cfg.CceAddonV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud cceAddon v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cce.%s.%s/api/v3/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "cceAddon", "v3", t)

	// test for cciV1Client
	serviceClient, err = cfg.CciV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud cci v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cci.%s.%s/api/v1/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "cci", "v1", t)

	// test for cciV1BetaClient
	serviceClient, err = cfg.CciV1BetaClient(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud cci v1 beta client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cci.%s.%s/apis/networking.cci.io/v1beta1/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "cci", "v1 beta", t)

	// test for FgsV2Client
	serviceClient, err = cfg.FgsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud fgs v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://functiongraph.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "fgs", "v2", t)

	// test for swrV2Client
	serviceClient, err = cfg.SwrV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud swr v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://swr-api.%s.%s/v2/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "swr", "v2", t)

	// test for BmsV1Client
	serviceClient, err = cfg.BmsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud BMS v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://bms.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
...
func TestAccServiceEndpoints_Storage(t *testing.T) {
	...
	serviceClient, err = cfg.BlockStorageV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud blockStorage v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://evs.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "blockStorage", "v2", t)

	// test for blockStorageV21Client
	serviceClient, err = cfg.BlockStorageV21Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud blockStorage v2.1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://evs.%s.%s/v2.1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "blockStorage", "v2.1", t)

	// test for cbrV3Client
	serviceClient, err = cfg.CbrV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud cbr v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cbr.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "cbr", "v3", t)

	// test for	sfsV2Client
	serviceClient, err = cfg.SfsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud sfs v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://sfs.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "sfs", "v2", t)

	// test for sfsV1Client
	serviceClient, err = cfg.SfsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud sfs v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://sfs-turbo.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "sfs", "v1", t)

	// test for csbsV1Client
	serviceClient, err = cfg.CsbsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud csbs v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://csbs.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "csbs", "v1", t)

	// test for vbsV2Client
	serviceClient, err = cfg.VbsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud vbs v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://vbs.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
...
func TestAccServiceEndpoints_Network(t *testing.T) {
	...
	serviceClient, err = cfg.NetworkingV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud vpc v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://vpc.%s.%s/v1/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "vpc", "v1", t)

	// test endpoint of vpc v3
	serviceClient, err = cfg.NetworkingV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud vpc v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://vpc.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "vpc", "v3", t)

	// test endpoint of network v2 service
	serviceClient, err = cfg.NetworkingV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud networking v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://vpc.%s.%s/v2.0/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "networking", "v2.0", t)

	// test endpoint of nat gateway
	serviceClient, err = cfg.NatGatewayClient(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud nat gateway client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://nat.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "nat", "v2", t)

	// test endpoint of nat gateway v2.0
	serviceClient, err = cfg.NatV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud nat gateway v2.0 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://nat.%s.%s/v2.0/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "nat", "v2.0", t)

	// test endpoint of elb v2.0
	serviceClient, err = cfg.ElbV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud ELB v2.0 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://elb.%s.%s/v2.0/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "elb", "v2.0", t)

	// test endpoint of elb v3
	serviceClient, err = cfg.ElbV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud ELB v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://elb.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "elb", "v3", t)

	// test endpoint of loadbalancer(elb v2)
	serviceClient, err = cfg.LoadBalancerClient(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud ELB v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://elb.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	compareURL(expectedURL, actualURL, "elb", "v2", t)

	// test the endpoint of fw v2 service
	serviceClient, err = cfg.FwV2Client(HCSO_REGION_NAME)
	if err != nil {
	...
	serviceClient, err = cfg.DnsV2Client(HCSO_REGION_NAME)
	if err != nil {
	...
	serviceClient, err = cfg.DnsWithRegionClient(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud DNS region client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dns.%s.%s/v2/", HCSO_REGION_NAME, cfg.Cloud)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.VPCEPClient(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud VPC endpoint client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://vpcep.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.ErV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating ER v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://er.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DcV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating DC v3 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dcaas.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
...
func TestAccServiceEndpoints_EnterpriseIntelligence(t *testing.T) {
	...
	serviceClient, err = cfg.MrsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud MRS v1.1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://mrs.%s.%s/v1.1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.MrsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud MRS v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://mrs.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.SmnV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud SMN client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://smn.%s.%s/v2/%s/notifications/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.CdmV11Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud cdm client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cdm.%s.%s/v1.1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DisV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud dis client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dis.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DisV3Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud dis client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dis.%s.%s/v3/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.CloudtableV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud cloudtable client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cloudtable.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.CloudStreamV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud cloudStream client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://cs.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.CssV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud css client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://css.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DliV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating dli css client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dli.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DliV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating dli v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dli.%s.%s/v2.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
		t.Fatalf("dli endpoint: expected %s but got %s", green(expectedURL), yellow(actualURL))
	}
	...
	serviceClient, err = cfg.DwsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating dws css client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dws.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DwsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating dws v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dws.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.GesV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating ges css client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://ges.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.MlsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating mls css client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://mls.%s.%s/v1.0/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.ModelArtsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating ModelArts v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://modelarts.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.ModelArtsV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating ModelArts v2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://modelarts.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.DataArtsV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud DataArts v1 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://dayu.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.WorkspaceV2Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud Workspace V2 client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://workspace.%s.%s/v2/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
...
func TestAccServiceEndpoints_Edge(t *testing.T) {
	...
	serviceClient, err = cfg.IECV1Client(HCSO_REGION_NAME)
	if err != nil {
...
func TestAccServiceEndpoints_Others(t *testing.T) {
	...
	serviceClient, err = cfg.MaasV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud MAAS client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://oms.%s.%s/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
	...
	serviceClient, err = cfg.SmsV3Client(HCSO_REGION_NAME)
	if err != nil {
	...
	serviceClient, err = cfg.AomV1Client(HCSO_REGION_NAME)
	if err != nil {
		t.Fatalf("Error creating HuaweiCloud AOM client: %s", err)
	}
	expectedURL = fmt.Sprintf("https://aom.%s.%s/svcstg/icmgr/v1/%s/", HCSO_REGION_NAME, cfg.Cloud, cfg.TenantID)
	actualURL = serviceClient.ResourceBaseURL()
	if actualURL != expectedURL {
