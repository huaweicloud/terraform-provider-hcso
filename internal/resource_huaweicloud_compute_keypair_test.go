...
func TestAccComputeV2Keypair_basic(t *testing.T) {
	...
	resourceName := "hcso_compute_keypair.test"
...
func testAccCheckComputeV2KeypairDestroy(s *terraform.State) error {
	hcsoConfig := testAccProvider.Meta().(*config.Config)
	computeClient, err := hcsoConfig.ComputeV2Client(HCSO_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloud compute client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcso_compute_keypair" {
			continue
		}
