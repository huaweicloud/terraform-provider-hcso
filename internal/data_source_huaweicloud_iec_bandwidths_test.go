...
func TestAccIECBandWidthsDataSource_basic(t *testing.T) {
	resourceName := "data.hcso_iec_bandwidths.bandwidths"
...
resource "hcso_iec_eip" "eip_test1" {
  site_id = data.hcso_iec_sites.sites_test.sites[0].id
}

resource "hcso_iec_eip" "eip_test2" {
  site_id = data.hcso_iec_sites.sites_test.sites[0].id
  line_id = data.hcso_iec_sites.sites_test.sites[0].lines[1].id
}
`
...
data "hcso_iec_bandwidths" "bandwidths" {
  site_id = data.hcso_iec_sites.sites_test.sites[0].id
}
