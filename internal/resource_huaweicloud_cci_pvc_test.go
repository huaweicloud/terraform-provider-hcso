...
func TestAccCCIPersistentVolumeClaims_basic(t *testing.T) {
	...
	resourceName := "hcso_cci_pvc.test"
	volumeType := "ssd"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "namespace", HCSO_CCI_NAMESPACE),
...
func testAccCheckCCIPersistentVolumeClaimsDestroy(volumeType string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		client, err := config.CciV1Client(HCSO_REGION_NAME)
		if err != nil {
			return fmtp.Errorf("Error creating HuaweiCloud CCI client: %s", err)
		}
		for _, rs := range s.RootModule().Resources {
			if rs.Type != "hcso_cci_pvc" {
				continue
			}
			response, err := getCCIPvcInfoById(client, HCSO_CCI_NAMESPACE, volumeType, rs.Primary.ID)
	...
	return func(s *terraform.State) (string, error) {
		...
		if pvc.Primary.Attributes["volume_type"] == "" || pvc.Primary.ID == "" {
			return "", fmtp.Errorf("Unable to find the resource by import infos: %s/%s/%s",
				HCSO_CCI_NAMESPACE, pvc.Primary.Attributes["volume_type"], pvc.Primary.ID)
		}
		return fmt.Sprintf("%s/%s/%s", HCSO_CCI_NAMESPACE, pvc.Primary.Attributes["volume_type"], pvc.Primary.ID), nil
	}
}
...
`, rName, HCSO_ENTERPRISE_PROJECT_ID_TEST, rName, HCSO_CCI_NAMESPACE, volumeType)
}
...
resource "hcso_cci_pvc" "test" {
  ...
  namespace   = "%s"
  volume_type = "%s"
  volume_id   = hcso_obs_bucket.bucket.id
}
`, rName, HCSO_ENTERPRISE_PROJECT_ID_TEST, rName, HCSO_CCI_NAMESPACE, volumeType)
}
...
resource "hcso_sfs_file_system" "sfs_1" {
  share_proto  = "NFS"
  size         = 10
  name         = "%[1]s"
  description  = "sfs_c2c_test-file"
  access_to    = hcso_vpc.test.id
  access_type  = "cert"
  access_level = "rw"
...
resource "hcso_cci_pvc" "test" {
  ...
  namespace         = "%[3]s"
  volume_type       = "%[4]s"
  volume_id         = hcso_sfs_file_system.sfs_1.id
  device_mount_path = hcso_sfs_file_system.sfs_1.export_location
}
`, rName, HCSO_ENTERPRISE_PROJECT_ID_TEST, HCSO_CCI_NAMESPACE, volumeType)
}
...
resource "hcso_cci_pvc" "test" {
  ...
  namespace         = "%[2]s"
  volume_type       = "%[3]s"
  volume_id         = hcso_sfs_turbo.test.id
  device_mount_path = hcso_sfs_turbo.test.export_location
}
`, rName, HCSO_CCI_NAMESPACE, volumeType)
}
