...
# allow http
resource "hcso_networking_secgroup_rule" "allow_http" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 80
...
resource "hcso_lb_loadbalancer" "elb_1" {
  name          = var.loadbalancer_name
  vip_subnet_id = hcso_vpc_subnet.subnet_1.ipv4_subnet_id
}

# associate eip with loadbalancer
resource "hcso_vpc_eip_associate" "associate_1" {
  public_ip = hcso_vpc_eip.eip_1.address
  port_id   = hcso_lb_loadbalancer.elb_1.vip_port_id
}
...
resource "hcso_lb_listener" "listener_1" {
  ...
  protocol        = "HTTP"
  protocol_port   = 80
  loadbalancer_id = hcso_lb_loadbalancer.elb_1.id
}

resource "hcso_lb_pool" "group_1" {
  name        = "group_1"
  protocol    = "HTTP"
...
resource "hcso_lb_monitor" "health_check" {
  name           = "health_check"
  type           = "HTTP"
  url_path       = "/"
  ...
  timeout        = 5
  max_retries    = 3
  pool_id        = hcso_lb_pool.group_1.id
}

resource "hcso_lb_member" "member_1" {
  address       = hcso_compute_instance.instance[0].access_ip_v4
  protocol_port = 80
  weight        = 1
  pool_id       = hcso_lb_pool.group_1.id
  subnet_id     = hcso_vpc_subnet.subnet_1.ipv4_subnet_id
}

resource "hcso_lb_member" "member_2" {
  address       = hcso_compute_instance.instance[1].access_ip_v4
  protocol_port = 80
  weight        = 1
  pool_id       = hcso_lb_pool.group_1.id
  subnet_id     = hcso_vpc_subnet.subnet_1.ipv4_subnet_id
