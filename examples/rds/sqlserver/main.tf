locals {
  ...
  se_version_names = [for n in data.hcso_rds_engine_versions.default.versions[*].name : n if length(regexall("^\\d+_SE$", n)) > 0]
  # Filter the web editions
  web_version_names = [for n in data.hcso_rds_engine_versions.default.versions[*].name : n if length(regexall("^\\d+_WEB$", n)) > 0]
}

data "hcso_availability_zones" "default" {}

# Query an available engine version list of the SQL server database.
data "hcso_rds_engine_versions" "default" {
  type = "SQLServer"
}

# Query an available flavor of web edition using specified vcpus and memory numbers.
data "hcso_rds_flavors" "default" {
  db_type = "SQLServer"
...
data "hcso_rds_flavors" "ha_database" {
  db_type = "SQLServer"
...
resource "hcso_rds_instance" "default" {
  name              = var.single_instance_name
  flavor            = data.hcso_rds_flavors.default.flavors[0].name
  vpc_id            = hcso_vpc.default.id
...
resource "hcso_rds_instance" "ha_database" {
  name                = var.ha_instance_name
  flavor              = data.hcso_rds_flavors.ha_database.flavors[0].name
  vpc_id              = hcso_vpc.default.id
  subnet_id           = hcso_vpc_subnet.default.id
  security_group_id   = hcso_networking_secgroup.default.id
  ...
  availability_zone = [
    data.hcso_availability_zones.default.names[0],
    data.hcso_availability_zones.default.names[1],
  ]
